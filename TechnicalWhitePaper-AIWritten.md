...
| 羅馬數字輸入 | ✅（2.10.0+ Ctrl+\\）三種風格：字母、Unicode 大寫、Unicode 小寫 | ✅（4.1.0+ Shift+Cmd+R）1-3999 範圍，Unicode 大小寫 | 小麥注音：2025→MMXXV/ⅯⅯⅩⅩⅤ/ⅿⅿⅹⅹⅴ；威注音：Ⅰ、Ⅱ、Ⅲ/ⅰ、ⅱ、ⅲ |
| 就地加詞/刪詞/控頻 | 只支援加詞 | 加詞、刪詞、升降頻（Shift+Command+Enter 降頻） | 支援詞音不等長 |
| 選字窗刪詞/控頻 | 鍵盤交互，只支援刪詞、升頻 | 滑鼠右鍵選單刪詞、升降頻 |  |
| 候選字朗讀 | ✅（2.9.3+ 與 VoiceOver 聯動）使用 macOS 系統內建的 SQLite 輔助功能資料庫 | ✅（4.1.0+ 可設定）查詢使用者自訂關聯詞語資料庫 | 小麥例：「中國地方的中」 |
