...
|-------|----|----|----|----|
| macOS (13 Ventura - 至今) | [GitHub](https://github.com/vChewing/vChewing-macOS/releases), [Gitee](https://gitee.com/vChewing/vChewing-macOS/releases) | [GitHub](https://github.com/vChewing/vChewing-macOS/wiki/%E6%9B%B4%E6%96%B0%E5%B1%A5%E6%AD%B7), [敝站](./ReleaseNotes.md) | [GitHub](https://github.com/vChewing/vChewing-macOS/), [Gitee](https://gitee.com/vChewing/vChewing-macOS/) | 4.5.7 GM (July 29, 2026) |
| macOS (10.9 Mavericks - 12 Monterey) | [GitHub](https://github.com/vChewing/vChewing-OSX-Legacy/releases), [Gitee](https://gitee.com/vChewing/vChewing-OSX-Legacy/releases) | 參見下載頁面 | [GitHub](https://github.com/vChewing/vChewing-OSX-Legacy/), [Gitee](https://gitee.com/vChewing/vChewing-OSX-Legacy/) | 4.5.7-Legacy GM (July 29, 2026) |
...
> **⚠️**：請在升級到 macOS 14 Sonoma 之前**先把唯音輸入法升級到至少 3.5.4 版**，否則可能會在剛升級完系統之後首次使用唯音輸入法時出現整個系統畫面失去響應（長達數十秒）的故障。該故障當且僅當「使用者片語辭典目錄或磁帶檔案在 iCloud Drive 裡面」這類情形才會發生。該故障乃唯音輸入法的 GCD 設計不慎觸發 macOS 一處設計缺陷所使然。從 3.5.4 版開始，唯音不會再在這種情況下觸發該系統缺陷。如果您發現了其他與輸入法有關的故障，歡迎**[電郵提報](./BUGREPORT.md)**。
...
- [3.6.1 SP2] 允許自動匯入處理藉由雅虎奇摩輸入法匯出的 txt 格式的使用者片語辭典資料，另解決了與 CapsLock 有關的當機故障。
...
- [3.2.0] 調整了 txt 格式的使用者片語辭典檔案的載入算法，使其載入耗時減半。
- [3.1.1-3.1.3] 取消了在就地輪替候選字時的使用者半衰記憶模組觀測行為。另附其它軟體故障修正與體驗改善。
- [3.1.0] 新增注音反查功能。
- [3.0.0] 換裝了全新的使用者片語編輯器。
...
13. 允許使用者片語辭典內的讀音以「漢語拼音+數字標調」的方式記錄（會被自動辨識）。
...
唯音 v1.8.8 版開始截至 v4.2.1 為止允許偵測「Shift 按鍵單次敲擊」得以實作與此有關的中英模式切換功能。該敲擊偵測模組承襲自 Qwertyyb 的[業火五筆輸入法](https://github.com/qwertyyb/Fire/)（MIT 授權）。不過，該模組要求至少 macOS 10.15+ 才可以正常運作。

唯音 4.2.2 版開始，該模組換成了唯音輸入法自己重寫的版本。
...
一、自唯音 2.5.0 版開始引入的全新的 FolderMonitor 模組（用來監視使用者片語辭典目錄的內容變化，以便就地重新載入）在嘗試監視的目錄不存在的時候被啟動的話，會崩潰掉輸入法。之前 Zonble 的 FSEventStreamHelper 有沒有這個特徵，我就不知道了，也沒測試過。然而，問題出在輸入法剛啟動時的檔案操作判斷邏輯上了：在尚未完成使用者片語辭典讀入的步驟的前提下，就開始了對使用者片語辭典目錄的「檔案變動事件有無變化之情況」的監視。// FolderMonitor 自唯音輸入法 4.2.3 版開始換成了開發者自己重寫的版本，擁有更強的 Sandbox 相容性。
