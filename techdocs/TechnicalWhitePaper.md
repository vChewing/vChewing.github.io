...
>
> 更新註記（2026-09-22）：唯音側已更新至 v4.8.4（2026-09-22 發行）；小麥注音側對照 v3.1.1（2026-09 版，主要為語彙資料更新）。本次修訂涵蓋已確認的過期敘述：系統需求、原始碼路徑、注音排列計數與各項版本標記。
...
以下列出唯音自自立分支後至 v4.8.4 之關鍵里程碑（日期以實際開發活動階段為準，非全部為正式發佈日）:
...
差異項 | 小麥注音 (3.1.1) | 唯音 (4.8.4) | 補記
-- | -- | -- | --
資料截取日期 | 2026-09-22 | 2026-09-22 | 小麥注音側依 v3.1.1、唯音側依 v4.8.4 更新
...
偏好設定視窗 | 自 3.1 起改為 SwiftUI（Basic／User Phrases／Advanced 三頁籤） | 採用唯音自己訂製的類似 SwiftUI 的 DSL 語言來通過 AppKit 構建視窗。<br />另外，主流發行版也有採 SwiftUI 實作另一版偏好設定視窗、且預設啟用 SwiftUI 偏好設定視窗。 |  
