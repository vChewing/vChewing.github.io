...
- 懶人包總結：中英混打模式的英數內容工具提示現會隨文脈挪位，並可用 Option+BackSpace 一次清空尚未遞交的內容；「以聲調鍵覆寫游標身後讀音」的上下文提示現可於偏好設定內關閉；修復了就地加詞與打字學習記憶模組搶佔檔案寫入的死鎖故障、以及加詞或控頻後未能立即生效的故障。
...
- 懶人包總結：盲文點字輸出現支援 ASCII 點字（SimBraille）遞交；新增「對接輸入客體時以內文提示顯示當前打字模式」的功能（預設關閉）；修復原廠辭典目錄預設空值被誤判為無效路徑而反覆彈窗的故障；修復第一聲讀音被跨聲調漸退記憶誤套用的故障；例行語彙資料更新。
- 體驗：[所有發行版] 盲文點字功能現支援 ASCII 點字遞交。ASCII 點字當且僅當在使用配套的字型的時候才會將 ASCII 編碼的點字內容顯示成點字。
- 體驗：[所有發行版] 新增了一個功能、允許輸入法在剛剛與新的客體軟體開始打字會話的時候以 0.7 秒的提示文字來顯示這個輸入法目前處於何種模式（磁帶模式／注音打字模式／拼音打字模式／狂拼模式／英文模式／CpLk 英文模式）。該提示所顯示的位置就是在文字輸入的位置附近。然而，客體軟體可能會在沒有接收文字輸入的時候向輸入法謊報這種位置，導致這個提示文字的位置可能會在比較奇怪的地方顯示。此功能預設為關閉，可在「偏好設定->行為設定」分頁勾選啟用。
- 修復：[所有發行版] 解決了唯音輸入法會嘗試使用「解讀沙箱書籤」的方式解讀原廠辭典目錄預設空值的故障。這個故障在 v4.7.3 被曝露，導致任何「沒有手動設定過原廠辭典目錄」的使用者們每次重新啟動輸入法時都能看到使用者辭典目錄存取問題的訊息。
- 修復：[所有發行版] 回退了唯音輸入法 v4.7.3 不慎引入的一個激進行為，該行為會使得輸入法在啟動過程中讓偏好設定視窗相關的模組過早地偵測使用者辭典目錄內的資料變化。
- 修復：[所有發行版] 修正了 v4.7.3 SP1 的漸退記憶問題修復不徹底所遺留的故障：無聲調記號的陰平聲調讀音（如「ㄕㄥ」）仍可能被帶調的跨聲調記憶（如「ㄕㄥˋ」的「聖」）誤套用於組句與選字。
- 辭典：[所有發行版] 例行語彙資料更新。
...
- 懶人包總結：狂拼模式體驗精修（未完成讀音敲標點不再蜂鳴、簡拼容錯查詢按詞值去重、重切候選拒絕過大建議）；選字窗讀音消歧義支援漢語拼音顯示。
- 體驗：[所有發行版] 狂拼模式下注拼槽尚有未完成讀音時直接敲標點鍵，不再以蜂鳴拒絕：輸入法會先將當前未完成讀音固化進組字器、再讓標點正常插入。
- 體驗：[所有發行版] 啟用「拼音並擊」時，選字窗的讀音消歧義區域亦以漢語拼音顯示，與組字區保持一致。
- 體驗：[所有發行版] 以 App Bundle 形式分發的安裝包在 macOS 10.13 為止的系統下，安裝程式的視覺設計（mesh 漸層背景）更新至與較新系統一致的觀感。
- 修復：[所有發行版] 鐵恨注拼引擎現可在狂拼模式下允許更長的拼音暫存區。
- 修復：[所有發行版] 狂拼簡拼容錯查詢的結果按「詞值去重、保留最高機率者」處理：同一詞同時存在「無調墊底」與「聲調專屬」兩種讀音鍵時，不再讓低權重條目把高權重條目擠出候選前列（如「我」不再被「喔」壓過）。
- 修復：[所有發行版] 狂拼候選窗的重切候選套用邏輯收斂：拒絕「超出組字器承接上限」的過大建議（避免候選與組字器既有讀音重疊而組出錯誤詞序）；首段讀音重合判定收斂為單段，避免多讀音桶下插入鍵數與覆寫跨度錯位。
- 研發：[所有發行版] 狂拼模式底層效能優化：簡拼匹配改以首字母桶索引、拼音 Trie 改以貪婪切分行走、狂拼組句 DP 的音節分數記憶化、辭典查詢緩衝改為 O(1) FIFO 頭部淘汰並擴大解析條目快取。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.7.1

- 懶人包總結：狂拼模式副駕選字窗不再響應 Emacs 熱鍵；修復了狂拼模式下與打字學習記憶內容套用有關的行為故障；Intel Mac 效能改良。
- 體驗：[所有發行版] 狂拼候選窗的「替代切分整詞」候選套用邏輯改良：當候選詞的前段讀音與組字器最後一鍵的讀音重合時，只插入重合段之後的讀音並覆寫整詞——避免「他」被重複複製成「他他們的」這類組句錯誤。
- 體驗：[所有發行版] 狂拼 copilot 前置候選按文字內容去重：同一詞同時來自「容錯查詢」與「組句橫跨節點」時只保留先出現者，候選窗不再重複陳列。
- 修復：[所有發行版] 修復了狂拼 copilot 候選窗顯示期間、字母鍵（如「zh／ch／sh」的第二個 romaji「h」）被 VIM／Emacs 風格 JKHL 鍵重詮釋轉成方向鍵、誤觸「未完成讀音固化」並誤開正常選字窗的故障。
- 行為：[所有發行版] 套用「ㄅ半」SCPC 懶人包設定時會自動關閉拼音打字模式，避免兩種模式衝突。
- 行為：[所有發行版] 組字引擎的笛卡爾積防禦預算依末代 Intel 硬體實測再校準——免聲調的 4 音節（含）以下長詞仍可整詞連續敲擊組出；5 音節以上的免聲調連打恢復縮減防禦（需以聲調或空格劃分音節），兼顧長詞組句與系統防卡死。
- 辭典：[所有發行版] 例行語彙資料更新。
...
- 懶人包總結：拼音打字現可使用狂拼模式（類似於搜狗拼音的體驗）；對修飾鍵變更事件改採放行策略；修補康熙轉換模式惡例。
- 修復：[所有發行版] 修復了繁體中文模式下啟用康熙漢字轉換輸出時無法正常打出「才」的故障。原因：當代繁體至康熙繁體的轉換過程當中，「才->纔」是一對多轉換、且康熙繁體也使用「才」字。與「核」「參」有關的類似的轉換故障也得到了修補。
- 修復：[所有發行版] 現對所有 flagsChanged 鍵盤事件在處理完畢之後一律放還給系統。這有利於諸如「虛擬機」「遠端桌面」等「跨電腦傳遞鍵盤事件」的場合、不會讓另一端的電腦誤以為你沒有摁修飾鍵。
- 體驗：[所有發行版] 新增了「狂拼模式」，可在唯音輸入法偏好設定的「行為設定」頁面啟用。
  - 該功能專門服務於那些習慣了「昇陽拼音（SunPinyin）」「智能狂拼（ChineseStar）」或「搜狗拼音（Sogou）」連續組句輸入的使用者。
  - 該功能目前僅對拼音輸入（任一拼音排列）生效，對磁帶、逐字選字（SCPC）、內碼、漢音鍵盤符號與羅馬數字等輸入方法不適用。
  - 啟用後，連續敲擊拼音字母即可組句，無需逐音節敲聲調或空格鍵確認。
    - 注拼槽內尚未完成的拼音會以語言模型試算整句、即時預覽於組字區，尾段候選窗亦同步顯示，可循「Shift+選字鍵」就地選字；
    - 預覽期間原始拼音改以提示框顯示（候選窗顯示時自動讓位、避免重疊）。狂拼預覽中按空格會將當前讀音插入組字器（不再兼任候選輪替）
    - ；注拼槽尚有未完成拼音時按 Enter 亦僅固化當前讀音（或就地確認高亮候選）並停留於組字狀態，待注拼槽清空後再按 Enter 才遞交整句。
  - 值得一提的是：唯音輸入法的原廠辭典索引是以漢語注音構築的。得益於 Tekkon 鐵恨注拼引擎的功能豐盈度，唯音輸入法的拼音模式（包括對此服務的狂拼模式）允許您在使用除了漢語拼音方案以外的其他（受鐵恨注拼引擎支援的）拼音方案時也能享受到上述其他輸入法的體驗。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.6.2

- 懶人包總結：改良了對巨型使用者片語檔案與巨型 CIN 磁帶表格檔案的讀寫效率。
- 行為：[所有發行版] 整個使用者辭典體系在自動重新整理格式等寫入操作時不會再自動將不合規的 UTF8 Bytes 換成 U+FFFD。
- 行為：[所有發行版] 唯音輸入法不再同捆 SQLite Amalgamation，而是改用 macOS 系統內建的 SQLite Dylib。目前只有「匯入奇摩輸入法的使用者片語資料」的功能會需要用到 SQLite。
- 行為：[所有發行版] 重構了部分與偏好設定畫面有關的 UI 元件實作，減少不必要的副廠程式庫依賴。
- 研發：[所有發行版] 棄用了迄上一個版本為止所使用的 LineReader 程式庫。受此影響的 CIN 磁帶檔案逐行讀入行為現已改用 UTF8 bytes 掃描之手段、免除了海量不必要的 `O(n²)` 運算。
- 研發：[所有發行版] 對與使用者片語有關的資料模組（包括格式整理模組）做了重構，盡量減少對 `String` 物件的操作、轉而使用對 UTF8 bytes 的直接操作。
- 辭典：[所有發行版] 例行語彙資料更新；全字庫資料更新至 2026-08-10。

## 4.6.1

- 懶人包總結：全專案多處子模組重構，藉此提升對記憶體的利用效率、進一步降低輸入法自身的專有記憶體佔用量。
- 研發：[所有發行版] 通知飄窗模組摒棄了「多個 NSWindow 孤立副本輪流出現，每條訊息開一個新的 NSWindow」的設計，轉而使用「單個透明的 NSWindow 搭配三個固定的可以重複利用的 NSView 子視圖」的設計方式、以求究極的記憶體利用效率。
- 研發：[所有發行版] MainAssmebly 模組現在會對「JIS / 康熙字」轉換器的結果做快取，至多 4096 筆。
- 研發：[所有發行版] 護摩組字引擎引入下述修正：
  - 修復了在組句器內的每個 Gram 的標識「沒能在分配時確保唯一性」的設計缺陷；
  - 改良了與 Gram Node 有關的分配方法；
  - Gram Node 現在不再是 class 而是 struct，將與此有關記憶體壓力由 heap 轉移至 stack，藉此消滅與此有關的 heap storm。
  - 亦引入了其他從 Swift API 細則特性層面上可以提升記憶體利用率的設計。亦引入了其他從 Swift API 細則特性層面上可以提升記憶體利用率的設計。
- 研發：[所有發行版] 輸入法語彙資料模組亦引入了從 Swift API 細則特性層面上可以提升記憶體利用率（減少 heap storm）的設計。
- 研發：[所有發行版] 田所選字窗的 Candidate Pool 後端資料模組亦引入了從 Swift API 細則特性層面上可以提升記憶體利用率（減少 heap storm）的設計，包括對 CandidateCellData 的結構化處理。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.6.0

- 懶人包總結：在辭典資料索引層面進一步節約了記憶體佔用；新增對 CIN v2.7 磁帶表格標準的支援；允許略過特定建置版本的軟體更新通知。
- 體驗：[所有發行版] 軟體版本更新通知視窗會顯示「略過此版本」按鈕、會叫停與該建置版本編號有關的今後的自動更新通知。略過紀錄以建置編號為依據，下一個建置編號的新版仍會照常提醒；使用者若想重新收到被略過版本的提醒，可手動執行一次「檢查更新」。
- 體驗：[所有發行版] 優化了原廠辭典資料模組、使用者片語資料模組、磁帶模組的「索引資料」的記憶體佔用，效果拔群。
- 體驗：[所有發行版] 磁帶模組現適配 CIN v2.7 標準、規定了「花牌鍵（單個或多個連續的萬用通配字元）」與「任意單個通配字元」這兩種不同的通配字元。這將允許行列輸入法的 CIN2 磁帶表格在唯音輸入法內支援「Shift+?」通配字元（僅能匹配一個任意字根），但需要 Gontera 親自更新行列輸入法的 CIN2 磁帶表格檔案。迄今為止的花牌鍵現在也可以作為初始字元鍵入到組筆區內。當磁帶檔案內的定義（無論是功能定義還是資料對應）使得該磁帶會響應「Shift+?」這個按鍵配對時，磁帶模組會在 Quick Candidate（急速候選）模式下停用「Shift+? 呼叫服務選單」的特性。
- 辭典：[所有發行版] 例行語彙資料更新。
...
- 懶人包總結：徹底重構了打字會話體系與 InputMethodKit 的交互方式，以適應 MRC 時代的記憶體管理需求。
...
- 懶人包總結：全專案在編譯階段停用 ObjC ARC，藉此試圖解決與 CpLk 輸入法切換「在部分電腦上卡頓」有關的其他極端情形。
- 研發：[所有發行版] 全專案在編譯階段停用 Objective-C ARC（不影響 Swift ARC），並將輸入法與 IMK 框架（IMK 打字控制器、IMKTextInput 等 ObjC NSObject）之間的所有互動改為純記憶體位址傳遞，完全繞過 ObjC ARC 的 retain/release 開銷。IMK 打字控制器現已不含有任何 stored property，其 alloc / dealloc 過程不再觸發任何 ObjC ARC 操作。此舉旨在徹底解決部分電腦上仍舊殘留的「透過 CapsLock 反覆切換中英輸入法時出現明顯卡頓」的問題。若此版仍未改善，請務必電郵回報，開發者將考慮以「無 ARC / GC」之語言重寫 IMK 交互層。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.5.4

- 懶人包總結：針對「就地輪替候選字詞」的功能引入了「弱輪替」特性；修復了倚天傳統鍵盤佈局無法正常中英混打故障；對調了 VIM 熱鍵「J / K」的行為。
...
- 懶人包總結：允許對逐字選字模式強制單行選字窗佈局；允許修改浮動組字窗的字號；ㄅ半懶人包按鈕行為調整。
- 修復：[所有發行版] 修復了在使用 CIN2 Quick Candidates (比如行列輸入法) 時在選字窗內出現了多餘的無意義的 fake reading disambiguation data 的情況。這些多餘的資訊會導致使用者產生無端的困惑。
- 體驗：[所有發行版] 在「選字窗設定」新增了兩個選項：「對逐字選字模式強制單行選字窗佈局」與「浮動組字窗字號」，以滿足與此有關的使用者習慣需求。
- 體驗：[所有發行版] 對偏好設定畫面的「我姓ㄅ」ㄅ半懶人包設定懶人包按鈕做了一些處理，使之套用一些其他的追加行為特性設定：選字鍵會被自動設為「123456789」且在逐字選字模式下的選字窗會「始終保持單行/單列且不展開」「使用縱向陳列的選字窗」。
- 體驗：[所有發行版] 改良了選字窗的美術表現。由於眾所周知 macOS 26 的 Liquid Glass 爛到透骨的原因，唯音輸入法從這一版更新開始僅對 macOS 27 開始的系統啟用 Liquid Glass 特性。
- 研發：[所有發行版] 繼續改良原廠辭典模組的載入速度與記憶體利用效率。
- 研發：[所有發行版] 繼續改良整個輸入法的辭典檢索效率。
- 研發：[所有發行版] 解決了被 Swift 6.4 篩查出的一些 concurrency 隱患。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.5.2

- 懶人包總結：改良了磁帶模組對 CIN / CIN2 輸入法表格檔案的載入速率與記憶體佔用；選字窗滑鼠滾輪行為調整；介面文字調整＆輸入法選單內容順序調整。
- 體驗：[所有發行版] 將唯音輸入法偏好設定的選單入口移至輸入法選單的最頂端，以確保使用者能夠在第一時間內注意到這才是唯音輸入法偏好設定、而不是被系統錯誤引導至鍵盤設定畫面內的空白頁面。
- 體驗：[所有發行版] 改良了磁帶模組對 CIN / CIN2 檔案的載入效能與記憶體利用效率。
- 修復：[所有發行版] 調整了部分介面用詞及翻譯，以減少歧義。
- 行為：[所有發行版] 滑鼠滾輪對「可以展開且尚未展開」的選字窗的行為改為「僅展開選字窗」，以更符合使用者的操作直覺。
- 研發：[所有發行版] 輸入法選單頂端區域新增贊助鏈接，以增加開發者收到贊助的概率。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.5.1 SP1

- 懶人包總結：就地加詞/控頻時不再遲鈍；選字窗現正式支援滑鼠捲動；允許以空格鍵就地輪替候選字；新增ㄅ半模式配置專用懶人包按鈕。
- [SP1] 修復：[所有發行版] 緊急修復了選字窗在系統尚未啟用暗黑模式的情況下可能會出現的「部分候選字詞的文字顏色與選字窗背景顏色幾乎相同」的故障。
- 修復：[所有發行版] 解決了在就地加詞時可能會導致輸入法卡頓一兩秒的故障。該故障往往在使用者片語辭典檔案記錄超過五千筆的時候發生。
- 體驗：[所有發行版] 對偏好設定畫面的部分選項的文字描述做了消歧義處理。另給偏好設定畫面新增了「我姓ㄅ，ㄅㄛㄅ，愛打ㄅ半的傳說，要打ㄅ半就點我」懶人包按鈕，簡化了ㄅ半輸入模式的配置體驗。
- 體驗：[所有發行版] 田所選字窗進化到第四代（研發代號「我修院」），原生支援類似於 IMK 選字窗那樣的滾動條捲動體驗。
- 行為：[所有發行版] 將空格鍵行為設定改為三態選項（插入空格 / 呼出選字窗 / 輪替候選字），且輪替模式下 Shift+Space 可反向輪替。
- 行為：[所有發行版] 將輸入法的自盡閾值（記憶體最大佔用量）由 1024MB 提升到 1536MB，以因應 macOS 26 開始的系統現狀。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.5.0

- 懶人包總結：辭典載入效率大幅提升；改良了一些與「逐字選字模式（ㄅ半）」有關的預設行為，減輕使用者心智負擔；新的工具提示不會再被前一個臨時工具提示打斷。
- 體驗：[所有發行版] 針對 Intel mac 大幅度改良了原廠辭典與使用者片語辭典資料的載入效能。
- 行為：[所有發行版] 修改了一些選項的預設行為，以改良「逐字選字模式（ㄅ半）」的使用者體驗。
- 修復：[所有發行版] 集中修復了幾處本地化翻譯失效的故障。
- 修復：[所有發行版] 修復了新的恆久工具提示會被前一個臨時工具提示所依賴的倒數計時器打斷顯示的故障。
- 研發：[所有發行版] 簡化了 Typewriter 模組內的打字調度模組等模組所需的 protocol 層級，使之更易於維護。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.4.7

- 懶人包總結：偏好設定畫面現支援關鍵字檢索功能，以便迅速找到想要調整的設定選項。另針對原廠辭典調整了「ㄗㄞˋ」讀音下「再」的原廠辭典頻率、使之僅次於「在」。
- 體驗：[所有發行版] 唯音偏好設定畫面現支援關鍵字檢索功能，以便使用者更快地找到想要調整的設定選項。
- 修復：[所有發行版] 修復了被前一個版本不慎引入的部分 UI 文字翻譯故障。
- 修復：[所有發行版] 修復了辭典查詢模組與護摩組字引擎當中與權重排序有關的一些步驟缺失。
- 辭典：[所有發行版] 略微降低了「再（ㄗㄞˋ）」的原廠辭典頻率、使之僅次於「在（ㄗㄞˋ）」。

## 4.4.6

- 懶人包總結：修復了在重新載入使用者片語檔案時沒有淨空舊有臨時資料的故障；更換或重設使用者片語辭典目錄位置時現可允許繼承舊位置的使用者片語資料。
- 體驗：[所有發行版] 允許在重設或指定新的使用者片語辭典目錄時將舊位置的資料併入新位置。
- 修復：[所有發行版] 解決了在重新載入使用者片語檔案時沒有淨空舊有臨時資料的故障。
- 研發：[所有發行版] 統一了 Localization keys 的格式，便於今後的開發維護。
- 辭典：[所有發行版] 例行語彙資料更新；全字庫資料更新至 2026-05-06。

## 4.4.5

- 懶人包總結：修復了標記模式不顯示工具提示的故障；修復了在 Chrome 內切換打字會話場合時的遲鈍故障；現可正確處理使用者片語內的 tab 分隔記號；修補了一些中英混打功能的邊界情形。
- 修復：[所有發行版] 修復了在標記模式下不顯示工具提示的故障。該故障由 v4.4.4 版不慎引入。
- 修復：[所有發行版] 修復了打字調度模組與漸退記憶模組無法正確「生成記憶」「套用記憶」的一些常見邊界情形。這次修正應該能允許使用者「選一遍字就能記住」。但這種記憶仍舊會在一週內隨時間消退。如果想使其永久記憶的話，請手動加詞。
- 修復：[所有發行版] 對打字會話副本管理系統棄用了 NSMapTable，改用純 Swift 的 LRU Table。藉此可以修復該輸入法在早於 macOS 26.5 的系統下與 Chrome 的相容性故障。另：唯音輸入法對打字會話副本的複用策略在 macOS 26.5 開始失效，但開發者根據個人使用體驗推測 macOS 26.5 自身的 InputMethodKit 很可能已經對「使用者快速頻繁敲擊 CpLk 切換中英文輸入法」的情形做了一些效能優化。
- 修復：[所有發行版] 修復了使用者片語檔案裡面以 tab 構成的 delimiter 無法被辨識處理的故障。
- 修復：[所有發行版] 針對「中英文混合輸入回退」功能在某些已知的極端情況下的不良行為範式做了一些修正。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.4.4

- 懶人包總結：新增「濾除掉與輸入模式不相符的漢字候選」功能；田所選字窗現可顯示讀音消歧義資訊；緊急修復了浮動組字窗與 Ghostty 的相容性故障。
- 修復：[所有發行版] 修復了浮動組字窗與 Ghostty 和 Microsoft Office 2011 的相容性故障。該故障由前一版更新不慎引入。
- 體驗：[所有發行版] 田所選字窗現在會在需要對候選字詞做消歧義處理時額外顯示該候選字詞在組字引擎內的的讀音資訊。這個資訊只會在「需要顯示這種資訊」的候選字詞被高亮顯示時才會顯示。
- 體驗：[所有發行版] 在「唯音偏好設定 -> 辭典設定」畫面內新增「濾除掉與輸入模式不相符的漢字候選」功能開關。該開關在預設情況下是關閉的。無論是簡體中文使用者、還是繁體中文使用者群體當中，都有特定的人群不希望在選字窗內看到與當前繁簡輸入模式無關的候選字。他們需要這個開關。注意：該開關只會影響到萬國碼 1.0.1 範圍內的漢字。為了保證該功能的正常使用，敝輸入法隨附的倚天中文 DOS 系統的繁體中文候選字詞資料剔除了「体」這個簡體字，恐會影響到一些使用者對候選字順序的肌肉記憶。如果該功能仍有漏網之魚的話，那一定是倚天中文 DOS 系統的繁體中文候選字詞資料裡面仍有其他殘存的簡體中文特有漢字。
- 研發：[所有發行版] 對 IMEState 狀態類型解除了 associatedType 約束。從長遠角度來看，這有利於今後的跨平台功能開發。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.4.3

- 懶人包總結：緊急修復了倚天 26 鍵等動態注音排列功能壞掉的故障；中英混打現可藉由 Shift+Space 直接遞交 ASCII 英數與空格。
- 修復：[所有發行版] 修復了倚天 26 鍵等動態注音排列的打字故障。該故障由唯音輸入法 4.4.2 版不慎引入。
- 修復：[所有發行版] 修復了「浮動組字窗」與「工具提示視窗」不會及時地更新自己的視窗根座標位置的故障。但是，如果正在接收打字的軟體是 Electron 構建的話，則該修復無法起作用。Electron 可恨就可恨在「基本上沒有一個 Electron App 有認真實作過 NSTextInputClient API 的」。
- 體驗：[所有發行版] 針對「中英文混合輸入回退」功能引入 Shift+Space 行為：立即將當前組字區的內容以純 ASCII 的形式遞交出去，不做漢字注音輸入判斷處理。
- 研發：[所有發行版] 改善了原廠辭典在載入時就地構築注音反查資料時的效率。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.4.2

- 懶人包總結：對中英混打功能使用窮舉單元測試檢出且修復了上百個漢字讀音打字失敗的故障，顯著增強了該功能的可用性。
- 修復：[所有發行版] 針對「中英文混合輸入回退」功能做了下述體驗改進與調整。
...
- 修復：[所有發行版] 修復了後端辭典模組的庫存狀態查詢 API 在回報結果時「不考慮倚天中文DOS系統讀音」的設計缺陷。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.4.1

- 懶人包總結：新增了「中英文混合輸入回退」功能、實現中英混打；修復了無法用注音輸入假名的故障。
...
- 懶人包總結：換用了「護摩」次世代組字引擎；拼音模式允許免聲調連打；修復了 CIN 磁帶模式滿碼不自動組字的故障。
...
- 懶人包總結：針對 CIN2 磁帶模式追加檔案快取保底機制；也就由 iCloud Drive 代管的 Documents / Desktop 等鏡照目錄特性導致的磁帶檔案存取失敗的情況完善了錯誤提示內容。
- 修復：[所有發行版] 針對 CIN2 磁帶模式追加了新的保底機制。當使用者將磁帶檔案放在由 iCloud Drive 代管的 Documents / Desktop 等鏡照目錄、導致沙箱書籤在重新開機或系統更新後失效時，輸入法現在會自動回退讀取保存在 App Support 目錄內的快取副本，以提升磁帶模式的可用性與可靠性；若使用者在指定磁帶檔案或使用者片語辭典目錄時實際挑中的是 Symbolic Link，輸入法也會先解碼到來源位置再保存設定。
- 體驗：[所有發行版] 現在若使用者將 CIN2 磁帶檔案或使用者片語辭典目錄指定到 iCloud Drive 代管的 Documents / Desktop，輸入法會提供更精確的提示訊息，提醒使用者改用本機目錄；若不確定系統的 iCloud Drive 鏡照資料夾設定，也可依照提示向 Apple Support 洽詢。
- 體驗：[所有發行版] 修訂了與卸除輸入法有關的介面說明，方便使用者更快理解為何需要改採手動卸除。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.3.6

- 懶人包總結：與 macOS 26.4.1 有關的緊急相容性修復：修復了「在輸入法每次嘗試存取被設在 iCloud Drive 的使用者片語辭典目錄時，都會連同正在接收文字輸入的軟體一同僵死數秒」的故障。
- 修復：[所有發行版] 緊急修復了唯音輸入法的沙箱書籤管理系統與 macOS 26.4.1 的 iCloud Drive 的相容性故障。該故障會在使用者將使用者片語辭典目錄設在 iCloud Drive 時發生、讓輸入法每次嘗試存取使用者片語辭典目錄時都會連輸入法加上「正在接收文字輸入的軟體」一同僵死數秒。
- 體驗：[所有發行版] 自 4.1.4 版開始，唯音輸入法收緊了自身的 Sandbox 策略。帶來的副作用就是自己無權卸除自己了。但開發者一直沒注意到這個情況。這次的更新會優化與此有關的軟體介面文字提示，以便指引使用者前往該輸入法的 WIKI 頁面的對應頁面了解手動卸除方法。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.3.5

- 懶人包總結：緊急修復了「藉由標記模式加詞時會讓萬國碼異種文字選擇器混入使用者片語辭典」的故障；全字庫字音過濾器對單個漢字的候選字詞僅降頻且不再濾除。
- 修復：[所有發行版] 緊急修復了「在利用標記模式加詞時會導致加入到使用者片語辭典的漢字被摻入注音字型破音字標記資訊」的故障。該故障影響 4.3.4 版的使用者，且只會在啟用「在組字區同步反應 ButKo BPMFVS」時才會觸發。已經被不慎摻入該標記的使用者片語檔案可以交給 Claude Code / Codex 等 LLM、令其在所有受影響的記錄當中刪除所有「Unicode Variation Selector」即可。
- 體驗：[所有發行版] 從這一版開始，辭典設定當中的「僅繁體中文：原廠辭典僅返回符合全字庫讀音的結果」開關在開啟時，對由單個漢字組成的候選字條目不再作濾除處理，改為統一降權至 -9.5。這或可滿足部分使用者的臨時打字需求。
- 研發：[所有發行版] 步天歌繁簡轉換引擎升級至 v2.0，棄用 SQLite 資料存儲，約佔用 7MB 左右的記憶體空間。
- 辭典：[所有發行版] 全字庫資料更新至 2026-03-18 的版本。

## 4.3.4

- 懶人包總結：新增 ButKo BPMFVS 注音字型破音字標記的輸出支援；允許始終敲出倚天中文 DOS 系統能敲的漢字；改良使用者片語檔案與 CIN2 磁帶的載入速度。
- 體驗：[所有發行版] 改良了 CIN2 磁帶檔案與使用者片語檔案的載入速度與記憶體利用效率。
- 體驗：[所有發行版] 現在起，哪怕不啟用「對所有單漢字候選字強制施加倚天中文DOS系統排序」，輸入法也會將該模式特有的候選字補入選字窗內（會補到比較偏後的區段）。
- 體驗：[所有發行版] 新增對 ButKo BPMFVS 注音字型破音字標記的輸出支援，以便服務那些需要製作語文教材的人群。唯音輸入法對該功能的實作方式與其它輸入法完全不同，且使用方法也有差異。唯一的共同點就是系統得安裝符合 ButKo BPMFVS 標準的漢字字型。唯音輸入法需要在「唯音偏好設定 -> 行為設定」裡面將「Command+Option+Ctrl+Enter」的遞交內容改為「遞交 ButKo BPMFVS 注音標記」才可以用這個熱鍵遞交帶有破音字標記的組字區內容，且其「內容顯示有效性」取決於字型與 BPMFVS 標準資料自身的支援範圍。除非使用者同時手動啟用「在組字區同步反映 ButKo BPMFVS」這個開關，否則 ButKo BPMFVS 注音標記的正確破音字標記效果不會在內文組字區內正確顯示。任何與 ButKo BPMFVS 的字型有關的問題請閱讀 [JustFont 的專文](https://blog.justfont.com/2024/05/bpmf-font/)對此的講解說明。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.3.3

- 懶人包總結：將原廠辭典格式由 SQLite 升級成 VanguardTextMap 以便於今後的功能開發；修復了選字窗等 UI 模組的一些排版瑕疵。
- 修復：[所有發行版] 調整了輸入法偏好設定畫面當中的「服務管理」與「客體管理」這兩個頁面的一些佈局細節。
- 修復：[所有發行版] 修復了選字窗在部分場合下的行數控制行為與展開行為不準確的故障。
- 研發：[所有發行版] 棄用了 AppKit 的 NSTextField-based Label，改換了自家手動重新實作的 NSLabelView。Label 這種東西讓 NSTextField 兼任顯示的話，不但牛鼎烹雞，而且佔用的記憶體空間容量在 macOS 26 系統下也會變得非常誇張。現在 macOS 26.4 哪怕在開發軟體時啟用了 `UIDesignRequiresCompatibility` 也不會減少對記憶體的佔用，且佔用量相比 macOS 15 而言仍舊多了不少，就很頭痛卻無奈。
- 辭典：[所有發行版] 為了因應唯音專案今後的功能開發的靈活性需求，唯音輸入法對原廠詞庫棄用 SQLite 格式，轉而使用（正在研發的）先鋒引擎所使用的 TextMap 格式。注意：該格式雖為 Text 類型的格式，但其架構設計不允許使用者手動編輯，一編輯就會壞掉。該格式旨在允許先鋒引擎套件的 Trie 模組與唯音輸入法 LangModelAssembly 模組將整個檔案以 RawBytes 的形式糊在一個固定的記憶體區段上、每次查詢時直接讀取指定的記憶體區段。這種手段可以極大地減少記憶體空間佔用。唯音輸入法現階段不支援 Trie，但其繼任者（先鋒引擎套件）支援 Trie。這會允許拼音輸入模式「不用非得輸入聲調」，且使得像「華碩智障輸入法/搜狗拼音」那樣「不完整讀音輸入」的功能實作成為可能。本次版本更新並不包含語料更新。

## 4.3.2

- 懶人包總結：解決了輸入法打字會話與正在接收文字輸入的軟體的可能的連線故障；允許以 JSON 匯入/匯出偏好設定；新增選字窗動畫。
- 修復：[所有發行版] 解決了在部分早期版本 macOS 系統下的輸入法打字會話在剛剛開啟時「無法對接到正在接收文字輸入的客體軟體」的故障。該故障表現為「輸入法不處理使用者的鍵盤事件」。
- 修復：[所有發行版] 解決了在 Chrome 系瀏覽器下輸入法的打字會話可能會失去與 Chrome 的連線的情況。
- 修復：[所有發行版] 解決了一部分接收文字輸入的客體軟體可能會丟失與輸入法打字會話的連線的故障。
- 體驗：[所有發行版] 田所選字窗現引入動畫效果，可以在輸入法偏好設定內自行關閉。
- 體驗：[所有發行版] 現允許在輸入法偏好設定的「開發道場」畫面以 JSON 的形式匯入 / 匯出輸入法偏好設定。注意該資料格式並不包含任何牽涉到「檔案路徑」「目錄路徑」「UI語言設定」的內容。
- 研發：[所有發行版] 放棄對 InputMethodKit 的直接使用，改用經過唯音專案以 Header 重構的方法製作的 IMKSwift SDK 來完成與系統的輸入法體系的對接。這使得輸入法的開發過程更加具有對 Modern Swift Concurrency 的相容性。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.3.1

- 懶人包總結：貫徹了與 InputMethodKit 有關的 API 交互效能優化，以照顧 CapsLock 中英文切換時的順暢體驗。
...
- 懶人包總結：讓使用者片語辭典模組就檔案讀寫方面使用統一的 Queue 來完成任務，藉此解決一些會破壞使用體驗的衍生故障。
- 修復：[所有發行版] 讓使用者片語辭典模組就檔案讀寫方面使用統一的 Queue 來完成任務。這解決了兩個問題：「將 iCloud Drive 設為使用者片語辭典目錄後，可能會隨著時間推移而產生大量的重複檔案」以及「在新的客體（接收文字輸入的軟體）內開啟新的打字會話副本時，偶爾會梗死幾秒鐘」。
- 研發：[主流發行版] 對整個專案做了大手術，改用 Swift 6.2 且做了與此有關的多工處理技術迭代處理，以允許該輸入法的打字模組在 Windows / Linux 系統下建置、開發。利用了 Swift 6.2 的 Default Isolation 特性，將包括打字會話在內的所有內容都預設釘死在 MainActor 上。某些特種後檯任務（比如使用者片語辭典檔案讀寫）另論。註：Windows 版唯音輸入法的正式開發仍需等待 Swift 接下來在 Windows 平台的對 Static Linking Against Foundation 特性的支援，且需要相當的資金捐助以支撐開發者的脫產全勤開發之開銷，還需要專門針對 Windows 平台的一些特性做精細的交互設計與相容處理，總之絕非彈指一瞬的事情。
- 研發：[Aqua 紀念版] 為了與 macOS 10.9 Mavericks 相容，Aqua 紀念版仍繼續使用 Swift 5.10。但是，為了在行為上與主流發行版保持一致，所有與打字會話有關的 API 交互行為現在全都被預設釘死在 MainActor 上。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.2.5

- 懶人包總結：修復了與通知飄窗觸發時機有關的行為故障，也修復了與使用者片語辭典存取有關的一些故障。
- 修復：[所有發行版] 修復了部分修飾鍵在單次敲擊以切換中英文打字模式時「哪怕偏好設定裡面設定了允許此時的飄窗通知，也始終不出現通知飄窗」的故障。
- 修復：[所有發行版] 修復了使用者片語辭典格式整理工具在讀入的檔案體積為 0 byte 時會崩潰的故障。
- 修復：[所有發行版] 修復了輸入法的語言模型模組在讀入使用者片語辭典資料時沒有使用專用的執行緒隊列的故障。該故障可能會導致系統層面的檔案存取行為衝突。雖然系統自己會解決這個問題，但在諸如 iCloud Drive 等場合往往會產生大量重複的檔案。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.2.4

- 懶人包總結：調整了橫向多列選字窗排版行為；例行更新語料。
- 體驗：[所有發行版] 橫向多列選字窗的內容呈現邏輯現在與 macOS 內建的 IMK 選字窗完全一致。由於 FCITX5-mac、微信輸入法等副廠輸入法的橫向多列選字窗均採此排版行為的原因，唯音輸入法也決定採用該特性。
- 辭典：[所有發行版] 例行語彙資料更新，且同步更新了最後一次 CNS 資料確認日期時間戳。

## 4.2.3

- 懶人包總結：修復了對使用者片語辭典目錄內資料變化無法感知的故障；允許自訂通知飄窗的配色與行為；允許添入原廠辭典不存在的「漢字-讀音」配對。
- 修復：[所有發行版] 現重新允許使用者藉由使用者片語辭典新增「不存在於原廠辭典內的」「漢字-讀音」配對。
  - 有些使用者會用這種方式自訂一些記錄、試圖迅速輸入一些符號 / Emoji，應當視為正常需求。
- 修復：[所有發行版] 解決了在「原廠預設使用者片語辭典目錄」以及「iCloud Drive 使用者片語辭典目錄」內的使用者片語辭典內容變更無法被輸入法偵測到的故障。
...
- 懶人包總結：重構了與 Shift 鍵中英文切換功能有關的按鍵偵測邏輯；修復了與 macOS 10.15 和 macOS 11 的相容性。
...
- 懶人包總結：解決了輸入法偏好設定記不住的故障；修整了與打字學習模組有關的一些應用邏輯。
...
- 懶人包總結：整句注音打字模式之候選字排列現預設遵循倚天中文 DOS 系統之排序，藉此徹底消滅使用者自訂單個漢字候選字排序之需求。
- 行為：[所有發行版] 這一版將「允許對單個漢字進行升權或排除」的功能開關挪到了「唯音輸入法偏好設定->開發道場」頁面。
  - 當這個開關沒被勾選的時候，你無法再通過輸入法自身的任何功能對單個漢字進行升頻、降頻、排除的操作。
  - 當這個開關沒被勾選的時候，使用者片語辭典內任何對單個漢字的升頻降頻覆寫記錄都不會再起作用。
  - 開發者急籲使用者們不要輕易啟用該勾選功能，因為會對組字引擎的組句結果構成毀滅性的干擾影響。
...
- 體驗：[所有發行版] 在「開發道場」內新增了一個功能，允許使用者暫時停止使用者片語辭典對組字引擎的任何影響。開發者可能會讓使用者啟用該功能、便於讓使用者迅速評估其所遇到的組句故障的癥結是否位於使用者片語辭典內部。
- 修正：[所有發行版] 修復了「輸入法會忘記修復某些不準確的 UserDefaults 偏好設定值」的故障。
- 辭典：[所有發行版] 例行語彙資料更新，新增《絕區零》語料（角色名稱等）。

## 4.1.7

- 懶人包總結：重構了與 VIM / Emacs 熱鍵模擬有關的處理流程，完善了 JKHL 鍵在選字窗狀態的行為。
...
- 懶人包總結：允許今後在 macOS 27+ 系統下也能直接繼承奇摩輸入法的使用者自訂詞語資料；阻止磁帶模式干擾使用者片語操作。
...
- 體驗：[所有發行版] 通知飄窗版式更新。
- 修正：[所有發行版] 解決了那些涉及到「偵測某個詞語是否已經在詞庫」的各種功能會被磁帶模式影響的故障。這包括「新增詞語」「整理使用者片語檔案」「自奇摩輸入法匯入使用者自訂詞語」等情形。
- 修正：[所有發行版] 解決了某些與檔案 I/O 有關的崩潰故障風險。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.1.5

- 懶人包總結：對打字行為學習模組新增急速遺忘模式；重構了與注音和磁帶模式有關的打字邏輯。
...
- 懶人包總結：修復了「當前的打字會話副本會誤以為自己不是當前副本」的故障；縮減了沙箱存取例外的目錄範圍。
- 行為：[所有發行版] 縮減了沙箱存取例外的目錄範圍。因此可能需要您重新設定使用者片語辭典目錄與 CIN2 磁帶檔案目錄。輸入法在發現這兩個路徑設定目標無法存取時，現在會主動通知使用者。
...
- 懶人包總結：修復了無法在由系統給出的檔案保存視窗內正常打字的故障。
...
- 懶人包總結：修復了與候選字詞覆寫行為有關的一些故障；選字窗與浮動組字窗現照顧系統偏好主題色之設定。
...
- 懶人包總結：繼續精簡記憶體佔用；從打字會話體系徹底移除了 autoreleasepool，理論上應該不會再卡頓了。
...
- 懶人包總結：新增羅馬數字輸入模式與選字窗候選字唸讀功能。另附多項能耗改良，且為了規避系統層面的 Bug 而停用了 Liquid Glass 介面。
- 修正：[所有發行版] 修復了與輸入法會話管理體系當中對 ARC 的物件釋放管理行為有關的不良設計。該設計或可在某些電腦上導致操作遲鈍或能耗問題。
- 修正：[所有發行版] 在使用者片語檔案有關的檔案系統觀測模組內引入了防抖機制，藉此改良與此有關的能耗。
...
- 懶人包總結：針對輸入法的會話控制體系移除了一些過激的 autoreleasepool 處理，藉此減少輸入法的閒時功耗。
...
- 懶人包總結：解決了自 4.0.0 版開始至 4.0.4 為止的記憶體效能故障；減少了打字行為學習模組對硬碟的寫入頻次。
...
- 懶人包總結：修復了就地刪詞功能失靈的故障；選字窗熱鍵刪詞功能現在不再需要摁 Fn 鍵。
...
- 懶人包總結：修復了打字行為記憶模組（POM）對正常打字行為的劫持故障。修復了在就地加詞、控頻之後沒能正確清理對應的 POM 記憶的故障。
...
- 懶人包總結：新增了對 CIN2 v2.5 磁帶格式的支援。行列輸入法使用者得向老刀索取新版磁帶、方可使用行列詞彙之功能。
- 修正：[所有發行版] 繼續修復打字行為記憶「觀測記錄->打新句子時再套用」過程對某些極端情形無效的故障。因應該部分內容修正，Megrez 組字引擎亦更新至 v5.0.6 版。
...
- 懶人包總結：解決了選字窗選字會被使用者打字行為記憶觀測資料綁架而失效等故障。
- 修正：[所有發行版] 修復了選字窗選字行為「會被使用者打字習慣觀測模組的資料綁架」的故障。
- 修正：[所有發行版] 修復了打字行為記憶「觀測記錄->打新句子時再套用」過程在某些情況下無效的故障。
- 修正：[所有發行版] 所有與使用者片語資料有關的寫入操作現在均經過專門的 Actor 來完成，以防止在寫入資料到硬碟時出現 Data Race。這可以改善威注音的使用者片語辭典資料與 iCloud Drive 的相容性。
- 研發：[所有發行版] 因應上述部分內容修正，Megrez 組字引擎亦更新至 5.0.5 版。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.0.0

- 懶人包總結：解決了記憶體洩漏的問題。更新了臨時記憶模組與組字引擎，兩者均有更換算法。後端程式大翻修＆對 macOS 26 提供支援。
...
- 修正：[所有發行版] 重構了檔案偵測模組。該模組用於偵測檔案系統層面上的使用者片語辭典目錄內的資料變化情形，發現有變化就會重新讀取之。這次重構指在將相關的反應行為放在 MainActor 以外的處理排程內進行防抖處理。這項變更理論上可以讓輸入法在 iCloud 資料同步不順時不至於讓整個系統的所有 IMKInputClient 應用失去響應。
...
- 修正：[所有發行版] 汰換了之前的「基於 Lukhnos Liu & MJHsieh 的 Natural Logarithm 曲線算法所實現的」半衰記憶模組。取而代之的是以野獸常數「0.114514」構築的「偽二次多項式衰減」漸退記憶觀測模組（POM）。新的 POM 記憶模組不致力於模仿人類的記憶曲線，而是側重於短期記憶的有效性。如果有需要長期記憶且反覆使用的詞語的話，使用者更應該將其加入自己的威注音使用者片語辭典。這是威注音輸入法本體所移除的最後一個在算法上來自小麥注音的組件，所以從這一版開始威注音輸入法本體會移除與小麥注音有關的 attribution（唯一例外是 Aqua 紀念版所使用的安裝程式，不過嚴格上來講只與 OpenVanilla 這款軟體有關、不是小麥注音的專屬產物）。
- 修正：[所有發行版] 天權星組字引擎更換為 v5.0.3 版，啟用 DAG-DP 動態規劃算法來組句（不是 Vertex Topological 排序法）。本次組字引擎更新承襲了來自開發者的某個實驗專案的一些新特性，對記憶體佔用做了徹底的最佳化、且擁有與全新的漸退記憶觀測模組（POM）深度協作的能力。但須知 macOS 26 提升了對「視窗顯示」這種事務本身的基礎記憶體開銷，所以 macOS 26 使用者可能很難察覺到這次更新所帶來的記憶體佔用上的改良。
...
- 懶人包總結：更換了效能更優的 Dijkstra 組句算法；在選字窗顯示時，允許以 H / L 鍵移動組字區游標；可關閉蜂鳴音效。
...
- 研發：[所有發行版] 給天權星組字引擎（Megrez）換用了效能更優的 Dijkstra 爬軌算法、不再使用與小麥注音 Gramambular 2 同款的 DAG-Relax 爬軌算法。
- 辭典：[所有發行版] 例行語彙資料更新＋原神/星穹鐵道/鳴潮語料更新；全字庫資料更新為 v2025-01-13 版本。

## 3.8.6

- 懶人包總結：針對以 Electron 構築的客體應用強制啟用浮動組字窗、以規避可能的相容性問題。該特性可手動關閉。
...
- 懶人包總結：將記憶體使用上限閾值由 384MB 上調至 1024MB；解決了在點滅 CapsLock 鍵時「會有浮動組字窗在螢幕上殘留顯示」的故障，等。
...
- 行為：[所有發行版] 微調了針對使用者片語檔案內的單漢字字音配對的預設權重指派策略與控頻策略。
...
- 懶人包總結：緊急修復了一處與「SecureEventInput 濫用行為偵測模組」有關的記憶體洩漏故障。
...
- 懶人包總結：重新設計了針對單個漢字的控頻方法；新增選字窗內的服務選單功能、且可自訂；允許將組字區內容遞交為盲文點字陣列。
- 修正：[所有發行版] 修復了客體管理器視窗內的清單窗格「不顯示滾動條」的故障。
- 修正：[所有發行版] 修復了「SwiftUI 版偏好設定視窗內建的使用者片語編輯器」可能「無法成功監視該檔案的來自其它程式的修改」的一處 KVO 故障。
...
- 懶人包總結：內部程式維護更新；另有針對原廠辭典內容的一些累積問題修正。
...
- 懶人包總結：重新設計了「匯入奇摩輸入法自訂詞資料庫」功能的圖形介面交互方式；選字窗支援萬國碼字元的碼位資訊顯示。
...
- 修正：[所有發行版] 解決了在「匯入奇摩輸入法自訂詞資料庫」之後、偏好設定視窗內的使用者片語編輯器「不自動重新載入內容」的故障。
...
- 體驗：[所有發行版] 新增了一個功能選項，讓田所選字窗在顯示符號時將符號描述（萬國碼碼位點）顯示於字根反查清單的位置。該功能預設啟用。
...
- 懶人包總結：對偏好設定視窗做了精實案；允許以 J / K 鍵在選字窗內移動組字區內的游標；重構了輸入法的一部分內部模組。
- [SP1] 修正：[所有發行版] 緊急修復了 AppKit 版偏好設定視窗「無法更改當前編輯的使用者片語資料種類」的故障。
...
- 懶人包總結：解決了「SecureEventInput 濫用行為偵測模組」的幾處缺陷；允許自訂數字小鍵盤的輸入行為；允許在選字窗內移動組字區內的游標。
...
- 懶人包總結：新增對 CIN2 v2.4 磁帶格式標準的支援、且新增了代令系統；另有其他雜項問題修正。
...
- 行為：[所有發行版] 此次更新引入了全新的 InputToken 敲字代令處理系統。
  ...
  - 該敲字代令系統有著比較豐富自由的可訂製性，可以允許使用者自訂一些威注音輸入法不方便直接內建的功能、以滿足各自的私下需求。
    ...
    - 舉例說你想用愛迪生發明鎢絲燈泡的那一年（1879 年）來紀年的話，往使用者片語檔案裡面插入「`MACRO@DATE_YEARDELTA:-1879 ㄞˋ-ㄉㄧˊ-ㄕㄥ-ㄐㄧˋ-ㄋㄧㄢˊ -114.514`」這一行就可以了。因為用到了「-114.514」這個懲戒權重，所以這個敲字代令不會干擾到組字引擎的正常組字。
...
- 懶人包總結：輸入法原廠詞庫改用 SQLite 技術，大幅度減少記憶體佔用；另修復了 3.6.3 GM 版不慎引入的一個會讓磁帶系統徹底癱瘓的故障。
...
- 懶人包總結：輸入法原廠詞庫改用 SQLite 技術，大幅度減少記憶體佔用。
...
- 體驗：[所有發行版] **因應 Apple 在 MacBook Pro 產品線對 8GB 運行記憶體的病態執著，威注音輸入法全專案針對原廠辭典改採 SQLite 資料庫、將記憶體佔用從原先的約 120MB 銳減至大約 30MB 以內。**
  ...
  - 與記憶體佔用統計有關的特別說明：
    ...
    - 2. 使用者片語辭典以及 CIN 磁帶資料的記憶體空間佔用得另外計算。
...
- 懶人包總結：針對「在後檯濫用 SecureEventInput API 的執行緒」引入了主動偵測機制；允許自動匯入處理藉由雅虎奇摩輸入法匯出的 txt 格式的使用者片語辭典資料，另解決了與 CapsLock 有關的當機故障。
...
- 懶人包總結：引入「強化型組字區安全防護」功能。與此功能有關的詳情請洽「純靠北工程師7ci」。
...
- 懶人包總結：允許關閉田所選字窗的展頁特性、在剛叫出選字窗時就多行顯示。另有大量故障修正與內容維護更動。
...
- 修正：在新增使用者片語（或者控頻）的時候，不會再在漂白相關的半衰記憶資料時將當前輸入法繁簡模式對應的各自的半衰記憶模組搞錯。
...
- 行為：在試圖就地新增使用者片語時，輸入法會先檢查該詞音配對是否已被登記到詞彙濾除表當中。僅當發現沒被列入（過濾掉）時，才會新增使用者片語（或升頻）；如果有被列入（過濾掉）的話，則會先「僅」將該詞音配對從詞彙濾除表刪除。標記模式當中的工具提示也做了對應的調整。
- 行為：在就地加詞、刪詞、控頻的時候，不會在完成任務之後同時重新載入使用者片語辭典檔案和語彙排除表檔案，而是按需決定要重新讀取的資料。
- 行為：輸入法記憶體佔用封印上限由 512MB 改為 768MB，方便某些極個別人群對巨型使用者片語辭典檔案的需求。超過這個封印上限的話，輸入法會自動重新啟動。
- 研發：全專案將 `[self] in` 置換成 `[weak self] in`，將這一類記憶體洩漏的可能性掐死在搖籃當中。
- 研發：優化了對使用者片語檔案的 EOF 標記的檢查與修復方法，使之不再需要載入整個檔案便可完成任務。
...
- 懶人包總結：解決了一處與 macOS 14 有關的惡性相容性故障。
- 修正：解決了一個與 macOS 14 Sonoma 有關的惡性相容性故障：如果使用者片語辭典目錄或磁帶檔案在 iCloud Drive 裡面的話，在系統剛升級到 macOS 14 之後，威注音輸入法在使用時會觸發 GateKeeper 機制、系統會詢問使用者「是否允許威注音存取受 iCloud Drive 管理的檔案」。本次更新會解決輸入法在出現這個畫面時的「系統整個畫面操作失去響應長達幾十秒」的故障。該故障乃威注音輸入法的 GCD 設計不慎觸發 macOS 一處設計缺陷所使然。從這一版開始，威注音不會再在這種情況下觸發該系統缺陷。
...
- 懶人包總結：針對全系列受支援的 Intel Mac 機種重寫了 Cocoa 版田所選字窗，極大地改善了介面渲染效率、減輕記憶體佔用。
...
- 體驗：輸入法偏好設定的「使用者片語辭典檔案目錄」以及「磁帶檔案目錄」的自訂用「…」按鈕迎來一處新調整：當您摁住 Option 鍵再點這個按鈕的時候，會直接以 Finder 開啟該路徑。
- 體驗：將主流發行版特有的「修改使用者片語辭典檔案目錄」以及「磁帶模式」功能下放給威注音輸入法 Aqua 紀念版（v0.5.25）。
...
- 懶人包總結：支援 CIN v2.1 - v2.2 標準；徹底重構了 IMK 選字窗的按鍵事件處理流程。
...
- 懶人包總結：改用 JSON 作為原廠辭典格式，節省記憶體佔用與硬碟空間佔用；另修復 SwiftUI 田所選字窗在某些佈局下「無法顯示反查結果」的故障。
...
- 懶人包總結：引入了一些與注音輸入有關的累積改進；另有引入對劉又銘教授的擬音注音排列的支援。
...
- 懶人包總結：解決了在藉由 macOS 系統內建 Emoji 視窗輸入表情時會崩掉輸入法的故障；另有其它故障修正。
...
- 懶人包總結：解決了在某些包含同音異幅的候選字詞的選字窗內來回選字時的崩潰故障；新增韋氏拼音輸入支援。
...
- 懶人包總結：修復了「單個漢字無法借由標記模式降頻」以及「當游標前後的漢字的讀音相同的時候，在此時叫出的選字窗內會出現橫跨游標的候選字詞」的故障。
...
- 懶人包總結：允許在標記模式內以節點為單位移動「標記專用游標」；針對「Shift+字母鍵」新增了兩種行為模式。
- 修正：解決了「在組字區內實時預覽選字結果」的功能無法在浮動組字窗內生效的故障。
- 修正：解決了浮動組字窗「可能會因為游標位置的不同、而出現排版行高異動」的異常行為。
- 修正：解決了使用者片語編輯器在前一次更新當中「喪失了標記重複內容的能力」的故障。
...
- 懶人包總結：組字區會實時反應你當前選字窗內的高亮的候選字詞的選字效果，另有程式庫內容維護更新。
- 修正：解決了會將諸如「CapsLock+字母鍵」這種特殊的熱鍵組合「誤判為 Shift 單次敲擊」的故障。
- 修正：解決了使用者片語辭典格式整理模組「無法正確處理位於檔案中間的格式化標頭字串」的故障。
...
- 研發：對 LMMgr 語言模組管理單元做了結構調整，且引入了全新的 UserPhrase 結構單元、以簡化使用者片語寫入流程。
- 研發：就與候選字詞有關的 Tuplet 單位新增了參數名稱。
- 辭典：例行語彙資料更新＋原神語料更新。

## 3.4.1

- 懶人包總結：第三代田所選字窗，採 Cocoa 技術重新製作；修復了組字引擎在爬軌時會出現的記憶體洩漏故障；另有諸多小修正與功能更新。
...
- 懶人包總結：對 SwiftUI 版偏好設定介面做了大翻修；新增「\ / ¥ 鍵呼叫漢音鍵盤符號模式」選項、Shift+Command+[] 選字窗候選切換、Alt+Delete/BackSpace 快速刪除字詞節點；另調整 ESC 鍵與半衰記憶資料清除行為、修復注音糾錯設計與舊系統鍵盤佈局故障。
...
- 懶人包總結：新增內碼輸入模式、且允許以此模式連續輸入漢字。另新增漢音符號表與漢音符號鍵盤模式支援。
...
- 懶人包總結：新增對諸如英雄聯盟等硬體全螢幕電玩的輸入法介面顯示支援，且對田所選字窗的橫向排列模式做了排版調整。
...
- 懶人包總結：允許以 Alt+Shift+Enter 將當前組字區「先逐字插入空格、再遞交」，方便 Sibelius 使用者整理中文歌詞。
...
- 懶人包總結：完善 SHIFT 鍵單獨敲擊行為判斷模組（並移除開發道場內的淘汰選項）；允許單獨啟用左 SHIFT 鍵的中英切換功能、允許以 JIS 鍵盤的英數鍵切換中英文模式。
...
- 懶人包總結：修復了田所選字窗右鍵選單控頻/刪詞功能故障；更正「是否允許選字窗使用右鍵選單」的條件判定；針對 Shift 中英文切換模組追加按鍵條件判定以減少誤判。
...
- 懶人包總結：輸入法狀態為空時不再處理 Emacs 熱鍵，避免 Ctrl+V 等熱鍵輸入垃圾字元；允許使用者改用由 vChewing-VanguardLexicon 部署的原廠辭典檔案；SwiftUI 新版語彙編輯器新增幫助說明彈窗按鈕。
- 修正：在輸入法狀態為空時，停止處理 Emacs 熱鍵。這可以避免包括「Ctrl+V」在內的相關熱鍵「會輸入垃圾字元」的問題。
- 行為：允許使用者改用由 vChewing-VanguardLexicon 部署在輸入法沙箱目錄內的原廠辭典檔案。該特性會在未來威注音輸入法專案（萬一）終止維護時派上用場。
- 體驗：SwiftUI 新版語彙編輯器新增幫助說明彈窗按鈕、方便檢閱內容填寫範例。Cocoa 版語彙編輯器仍以工具提示來行使該功能。
- 辭典：例行語彙資料更新＋原神語料更新。

## 3.2.0

- 懶人包總結：調整了 txt 格式的使用者片語辭典檔案的載入算法，使其載入耗時減半。
- 體驗：對 txt 格式的使用者片語辭典檔案的分析引擎做了運算效率調整，現在只需要消耗不到 50% 的時間就能完成載入。
...
- 懶人包總結：取消了在就地輪替候選字時的使用者半衰記憶模組觀測行為。另附其它軟體故障修正與體驗改善。
...
- [SP1] 體驗：新版使用者片語編輯器現在會在使用者點按「整理」按鈕時按需載入非當前繁簡體模式的核心語彙庫資料（之前則是需要你先手動切換一遍簡體中文輸入、或者關掉按需載入的功能）。
...
- 懶人包總結：換裝了全新的使用者片語編輯器。
- [SP3] 修正：解決了 SwiftUI 版使用者片語編輯器的正文編輯區域的行高不等高的故障。
- [SP3] 修正：解決了 SP2 版在引入正文區域工具提示時對 SwiftUI 版使用者片語編輯器的正文編輯區域帶來的故障。因此，不得不針對 SwiftUI 版使用者片語編輯器犧牲掉這個工具提示。
- [SP3] 修正：解決了 Cocoa 版使用者片語編輯器在關閉時部分記憶體釋放不充分的問題。
- [SP2] 體驗：針對新版使用者片語編輯器的辭典正文編輯區域和權重填寫區域均新增工具提示、來指導使用者該怎樣填寫內容。
- [SP2] 體驗：當原廠詞庫尚未載入完畢的時候，現在輸入法不再阻攔功能鍵的輸入（包括空格鍵在內）。
- [SP2] 修正：修復了輸入法在修改過使用者片語辭典目錄之後「不自動讀入新目錄的內容」的故障。該故障存在於截至 3.0.0 SP1 版為止的所有版本的威注音輸入法內。
...
- 研發：因應新版使用者片語編輯器介面的功能需求，LangModelAssembly 語言模型總成模組做了功能升級。
...
- 懶人包總結：允許在田所選字窗內使用滑鼠右鍵選單控頻或排除指定的候選字詞、且盡量保持選字窗繼續顯示。
...
- 懶人包總結：解決了田所選字窗自 2.8.0 版以來一直存在的嚴重記憶體洩漏問題；允許所有客體應用共用「Shift 中英文模式切換」狀態、允許以 Ctrl+Command+Enter 遞交注音文/字根；記憶體佔用上限提升至 512MB。
...
- [SP1] 研發：考慮到某些極端情形（比如巨型磁帶等），從這次更新開始威注音的記憶體佔用上限閾值提升至 512MB。到了這個限制時，威注音會自動重新啟動。一般情況下，不記入使用者片語資料帶來的記憶體佔用的話，威注音佔用的記憶體空間大概是 80~110MB。
...
- 懶人包總結：取消了會話控制器自 2.8.8 版引入的副本管理方法，藉此解決長期存在的記憶體洩漏故障；記憶體佔用量超過 300MB 時輸入法會自動重啟並給出系統通知。
...
- 懶人包總結：徹底修復了「輸入法重啟之後除非重新切換輸入法或視窗、否則無法立刻使用」的故障；解決 Spotlight 敲字頭幾個鍵被提前遞交、繁簡切換時未完成注音被遞交等問題；更換更容易辨識的輸入法選單圖示。
...
- [SP1] 研發：現在，在使用田所選字窗時，如果啟用了輸入法的偵錯模式的話，則每次高亮選擇某個候選字的時候、都會在 console.app 內實時印出目前候選字的對應的萬國碼編號及字元介紹說明。
...
- 懶人包總結：不再原廠隨贈「Alt(Option)+符號鍵」的選單內容（可藉自訂語彙擴充）；修復 Ctrl+Command+Enter 遞交漢語拼音時連字符變成數字 1、Shift+Space 無法輸入全形空格等故障；多項動態注音排列與數字鍵輸入修正。
...
- 懶人包總結：允許在選字窗底部顯示可能的字根反查結果（僅對橫排輸入時的田所選字窗生效）。
...
- 懶人包總結：允許在選字窗底部顯示可能的字根反查結果（僅對橫排輸入時的田所選字窗生效）；新增 CIN2 格式的磁帶檔案支援，允許使用者以 CIN 碼表磁帶檔案給威注音擴充諸如嘸蝦米、五筆、倉頡這樣的筆畫輸入功能。
...
- 體驗：從現在開始，在手動再次啟用使用者片語辭典目錄內容監視的功能的時候，會立刻重新載入使用者片語檔案。
...
- 懶人包總結：客體管理器現允許同時選取多個用以登記的客體 App；將選字窗相關按鍵處理邏輯移至 InputHandler 模組。
...
- 懶人包總結：撤回了 2.8.7 版對鐵恨引擎引入的多餘修改；糾正「重啟輸入法之後非得切換當前視窗才能用」與「剛切到某 App 就呼叫波浪鍵選字窗時、選字窗突然消失」等故障；就 SessionCtl 引入副本管理能力；另調整上游著作權標記方式。
...
- 研發：讓田所選字窗在首次初期化的時候就自動從 UserDefaults 讀入選字窗尺寸。
...
- 懶人包總結：重新設計 Apple 動態注音鍵盤佈局的訊號翻譯機制，連帶解決 2.8.6 曝露的逐字選字模式敲字體驗失常與選字窗顯示遲滯等問題；解除偏好設定內與鍵盤佈局、注音排列有關的資料值配對限制；SwiftUI 偏好設定新增「大千傳統／倚天傳統／漢語拼音」便捷設定按鈕。
...
- 懶人包總結：因 LINE 不徹底遵守 IMKTextInput 協定，將 LINE 與 Steam 並列為強制啟用浮動組字窗的客體；合併並修復被廢版的 2.8.5 SP2/SP3 內容（含舊系統 CapsLock 相關功能移除、田所選字窗選字鍵鍵盤佈局指定等）。
...
- 體驗：將輸入法選單的「編輯自訂語彙」重命名為「編輯威注音自訂語彙」，以免與 macOS 原生提供的「編輯使用者片語辭典」混淆。
- 辭典：對包含有「褐」的詞均新增其陸規審音。另附語料更新。

## 2.8.5 SP1

- 懶人包總結：針對 macOS 10.15 與 macOS 11 移植了田所浩二橫版矩陣選字窗。
...
- 懶人包總結：引入全新的田所浩二橫版矩陣選字窗（僅限 macOS 12 開始的系統）；通知飄窗行為優化；CapsLock 英數模式體驗改進。
...
- 懶人包總結：徹底重新設計飄雲通知視窗（新通知恆出現在螢幕最右上角、舊通知下移並淡化）；重新設計產品圖示；將飄雲通知視窗、浮動組字窗、工具提示視窗等組件塞成 Swift Package。
...
- 懶人包總結：新增「按需載入簡繁體模式的原廠辭典資料」偏好設定選項；解除對 macOS 10.13 的 IMKCandidates 封印；專案大規模重構為多個 Swift Packages、PrefMgr 改為可副本化；最低系統需求提升至 macOS 10.13.4。
...
- 懶人包總結：解決了首次安裝後無限崩潰、「選字窗」「工具提示」「內文組字窗」在多螢幕下亂跑、App 格式安裝包被門衛機制隔離等故障；PKG 安裝包在 macOS 12 開始的系統下不再對錯誤位置的檔案執行刪除。
...
- 懶人包總結：浮動組字窗新增縱排書寫模式支援（僅 macOS 10.14 開始）；於客體管理器登記的客體應用不再能存取組字區內容，防止聊天軟體擅自拿組字區內容做敲字行為預測分析。
...
- 懶人包總結：允許為 Shift 切換到的英文輸入模式指定專用鍵盤佈局；針對 Steam 預設啟用浮動組字窗（限 20 讀音）且支援以客體管理器登記其它應用；就地增刪詞高亮顯示、JIS/康熙字轉換邏輯與工具提示繪製等多項修正。
...
- 懶人包總結：以全新的 NSAttributedTextView 重寫了工具提示視窗（支援縱排輸入、不再遮擋正在輸入的文字）；淘汰上游的 FSEventStreamHelper 與 VersionUpdateAPI、改用全新的 FolderMonitor 與 UpdateSputnik 模組。
...
- 研發：淘汰了 Zonble 的 FSEventStreamHelper，改用僅依賴 DispatchSourceFileSystemObject 的全新的 FolderMonitor 監視模組。由於該模組不會對由所屬應用自身產生的資料夾內容變化行為做監視，所以威注音補上了在每次就地增刪詞之後的「重新讀入使用者片語」的步驟。這也解決了迄今為止 FSEventStreamHelper 與「威注音的使用者片語辭典格式自動整理模組」彼此的行為衝突問題。
...
- 懶人包總結：正式宣告對 Emacs 熱鍵的支援（生效範圍含 IMK 選字窗）；引入全新的單結構態械系統、允許就地加詞應對「詞的字數與讀音數不相等」；淘汰大量上游模組（InputSignal、InputSourceHelper、NSStringUtils 等）；天權星組字引擎升級至 2.1.0。
...
- 懶人包總結：允許在開發道場內徹底停用對 Chrome 系瀏覽器的 Shift 鍵單次擊鍵判定措施；以 NSEvent 功能擴充取代上游的 KeyHandlerInput 實現。
...
- 辭典：將使用者半衰記憶檔案挪至「~/Library/Containers/org.atelierInmu.inputmethod.vChewing/Data/Library/Application Support/」，以「`vChewing_`」開頭。這樣的話，在使用原廠預設的使用者片語辭典目錄時，就不會在每次選字時都會觸發使用者片語辭典重載行為。
- 辭典：例行語彙更新。

## 2.2.0

- 懶人包總結：天權星組字引擎更新至 v2.0.3、修正上下文鞏固範圍過大的問題；JIS 轉換模式下漢字重複時自動套用疊字符號「々」（佐佐木→佐々木）；極大豐富波浪鍵分類符號選單。
...
- 懶人包總結：輸入法選單末尾新增「熱鍵指南」；Shift+BackSpace 與聲調鍵的行為調整為與 macOS 內建注音輸入法幾乎一致；修復以表情符號視窗插入符號時收到 null NSEvent 的崩潰故障。
...
- 懶人包總結：利用 Apple 內部 API 強制曝露的手段讓 IMK 選字窗可以調整字號、不再被頑固置於所有視窗底部；緊急解決 macOS 10.11-10.13 下無限連環崩潰；偏好設定介面全面捲動式設計與修飾鍵稱謂規範（CMD→Command、ALT→Option）。
...
- 懶人包總結：修復源自上游的產品設計缺陷——藉由選字窗選字時、預期範圍外的字詞會被自動重設（該缺陷波及多款知名副廠輸入法）；「徹底禁用 SHIFT 鍵切換中英文模式」開關挪至偏好設定體驗頁。
...
- 懶人包總結：可以使用 (Shift+)波浪符號鍵輪替選字窗頁面；針對 Chromium 系瀏覽器清單啟用 Shift 中英文切換相容性判定；開發道場內的實驗性 IMK 選字窗獲得翻行/翻頁、Tab 輪替等改進（選字鍵與字號問題仍無解）。
...
- 懶人包總結：偏好設定面板內引入「開發道場」頁面、專門放置實驗性功能（含 IMK 選字窗實驗支援、langIdentifier 保守治療方案）；Voltaire MK3 選字窗風格調整、恢復半衰記憶每次觀察存檔。
...
- 懶人包總結：針對 macOS 10.15 啟用 SwiftUI 新版偏好設定視窗；天權星引擎升級至 v2.0.1（選字時不會再重設周圍節點）；允許在使用者片語辭典目錄內以 plist 覆寫逐字選字模式的候選字順序。
...
- 辭典：允許在使用者片語辭典目錄內藉由「data-plain-bpmf-chs.plist」「data-plain-bpmf-cht.plist」對逐字選字模式下的候選字詞順序進行覆寫修改。該功能是為了近十年來已經養成上游傳統注音選字順序習慣的人群準備的，但威注音不考慮隨贈上游的傳統ㄅ半注音的辭典（在ㄒㄧㄠˊ讀音的記錄內把學放得比洨更優先是殺小），所以就採用這種功能設計、允許使用者自訂之。相關的自訂修改方法會在近日公開於威注音輸入法官方主頁。
- 辭典：載入使用者語言模組時，現在會根據某些功能模式的開關狀態按需載入。
- 辭典：針對所有類型的使用者片語檔案，新增了可以應對「以大寫字母書寫的漢語拼音」的能力。
- 介面：針對 macOS 10.x 的偏好設定介面視窗的工具列圖示現已更換為 macOS 11 開始的偏好設定介面所使用的圖示。
- 介面：針對 macOS 11 開始的偏好設定介面當中的選字鍵設定內容已經由「體驗」頁籤挪至「鍵盤」頁籤。
- 雜項：鐵恨引擎更新至 v1.2.9，優化了在將拼音字串轉注音時的效率。這有助於提升使用者片語檔案的載入速度。
- 辭典：例行語料更新。

## 1.9.0 SP1

- 懶人包總結：就地刪詞按鍵改為 BackSpace/Delete、Shift+Command+Enter 改為「詞彙降權」；純 Swift 天權星組字引擎更新至 v2.0.0（同步 Gramambular 2 全部變更、導入基於頂點運算的爬軌算法）；組字區內可輸入文字數量上限不復存在。
...
- 體驗：本次更新對就地增減詞模式有較大變動：就地刪詞的按鍵由 Shift+Command+Enter 改為「BackSpace 或 Delete」。從這一版開始，Shift+Command+Enter 的職能改為「將目前選中的現有詞彙的權重降低（nerfing）」。被降低權重的詞會出現在使用者片語辭典檔案內、且被附上「-114.514」這個惡臭的權重數值（爬軌函數聞到臭味就不會去考慮這個詞）。
...
- 體驗：因應此次天權星組字引擎更新，威注音輸入法也嘗試同步了 Lukhnos Liu 在上游針對使用者半衰記憶模組的觀測行為修正。此外，在針對節點進行覆寫時，會連同讀音一起用作覆寫判斷依據。如果說就這樣也還會出現怎麼敲也記不住的人名的話，那就請考慮將其就地添入使用者片語。
...
- 懶人包總結：新增以兩側 Shift 鍵開關英數輸入模式（僅 macOS 10.15 開始，移植自業火五筆輸入法）；可直接敲「日期」「時間」「星期」輸入當前日期時間與星期；組字區容量上限鎖定為 30 字。
...
- 懶人包總結：加入半衰記憶資料檔案差錯導致一選字就崩潰的救災機制（崩潰後自動清空半衰記憶檔案）；游標不再橫穿多音或多字單音節點；內文組字區下劃線按字詞節點分段顯示；允許以 BackSpace 編輯輸入錯誤的讀音。
...
- 懶人包總結：新增對星光注音排列的支援（業界第四款二十六鍵注音排列）；啟用被塵封多個版本的精業排列支援；鐵恨引擎更新至 v1.2.8。
...
- 懶人包總結：新增大寫漢字輸出模式（Ctrl+CMD+Shift+M，方便金融業者）；允許輸入全形阿拉伯數字；新增無限聯想功能；波浪符號鍵選單允許回退到上一個選單層級。
...
- 懶人包總結：徹底修正源自上游的「無法正確判斷按鍵是否來自數字小鍵盤區」的 Bug（僅選字窗出現時響應）；新增對 JIS 鍵盤的符號鍵支援；半衰週期由 1.5 小時調整為 6 小時（整個記憶週期五天多）。
...
- 懶人包總結：鐵恨注拼引擎升級至 v1.2.7，修正將拼音轉換成注音時無法正常處理陰平聲調的問題。
- 辭典：鐵恨注拼引擎升級至 v1.2.7，修正了在將拼音轉換成注音時無法正常處理陰平聲調的問題。現在，使用者片語辭典檔案內的漢語拼音當中的陰平聲調在被讀取的過程中可正確轉換為可被輸入法檢索到的注音字串。
- 程式：將 keyHandler 內一些不需要的代理函式全部移除。這些函式都是之前是威注音 1.5.x 早期版本內用來銜接 Gramambular 與 Swift 版 keyHandler 的函式，現在已經不再需要了。
- 例行語彙更新。

## 1.8.2

- 懶人包總結：輸入法僅允許同時出現一個選字窗副本（修復 1.7.4 起「選字時閃退」的提報）；鐵恨引擎 v1.2.6 引入「ㄧㄜ/ㄩㄜ→ㄧㄝ/ㄩㄝ」注音組合糾正策略、並允許在偏好設定內關閉該糾正。
...
- 懶人包總結：允許聯想詞模式下摁 Enter 確認當前選中的聯想字（Esc/BackSpace/Delete 可取消）；使用者片語檔案被刪除時自動生成並填寫原廠註釋；聯想詞模組新增多種索引鍵與檔案格式支援。
...
- 辭典：現在，當使用者片語檔案被刪除時，輸入法會自動生成新的檔案、且自動填寫原廠註釋內容（來說明這個檔案該怎麼用）。如果是使用者聯想詞辭典檔案的話，還會自動寫入原廠的聯想詞辭典內容。該辭典內容僅包含部分來自威注音輸入法原廠詞庫的固有詞彙。考慮到ㄅ半輸入法使用者群體的習慣，聯想詞的原廠辭典內容今後不會再作變更（除非發現其簡體中文版本的詞庫當中有錯誤，屆時會訂正）。
...
- 辭典：允許在使用者片語模組的辭典內給單筆記錄指定具體的權重數值（最好不要大於 0，除非你知道你在做什麼）。以半形英數「#」符號開頭的數值會被無視。不指定的話，預設權重為 0。
- 辭典：在辭典建置時新增了對辭典健康狀況的檢查環節，相關檢查結果可以用終端機殼層的管線指令輸出到指定的 txt 檔案當中（比如 bash / zsh 的話就會是在腳本執行語句後加上「> ./report.txt」，nushell 的話請參閱其手冊）。
- 辭典：使用者片語編輯器內建的拼音轉注音的功能現已移除，以免傷害到使用者自行添入的權重資料。
- 辭典：使用者片語辭典內的漢字讀音，哪怕寫成漢語拼音（必須帶數字聲調 12345），也能被輸入法在讀取時正常辨識。聯想詞語言模組會在輸入法請求資料的時候將索引鍵值複製一份轉換成拼音、然後同時檢索兩者，檢索結果排序優先度為「以注音標記的字音配對索引鍵 > 以拼音標記的字音配對索引鍵 > 純漢字索引鍵」；其餘的使用者語言模組則會在載入資料時直接將拼音索引鍵轉換成注音索引鍵之後、登記於自身的記憶體辭典內。
...
- 懶人包總結：因應天權星組字引擎升級，Alt+前後方向鍵可以節點為單位快速移動游標、Alt+Ctrl+前後方向鍵移至最結尾/最開頭；候選字處理單位改為「讀音+字串」鍵值配對；半衰記憶資料存放位置鎖定並新增清空選項。
...
- 體驗：工具提示內的注音讀音會在使用拼音並擊模式時顯示成教科書格式的拼音，且會在新增使用者片語時顯示選中區域的讀音內容。
...
- 懶人包總結：正式引入威注音自家的「步天歌」繁簡轉換引擎、取代 OpenCC 與既有的康熙字＆日本簡化字轉換模組（自此 C++ 內容清除乾淨）；鐵恨引擎更新至 v1.2.3、新增「ㄅㄨㄥ」等注音糾錯。
...
- 懶人包總結：允許在啟用半衰記憶模組建議的同時固定選字窗內的候選字排序；就地輪替候選字的順序改為固定排序、不再受半衰記憶模組影響。
...
- 懶人包總結：使用者半衰記憶模組回歸並大幅強化（單元圖陣列建議、候選字排序提前、JSON 格式存檔、觀測時自動整理辭典順序等）；波浪符號鍵選單可讀取新酷音 symbols.dat 自訂內容；選字窗內漢字受康熙/JIS 轉換影響並以括弧顯示轉換前漢字。
- 功能：從本次更新開始，使用者半衰記憶模組回歸、且相比上游的 C++ 半衰記憶模組而言新增下述功能。該模組不會在逐字選字模式下起作用。
  ...
  3. 半衰記憶模組資料會在每次敲字的手動選字行為發生時以 JSON 格式記入至使用者片語資料目錄下的「override-model-data-chs/cht.dat」內。雖可明文編輯，但編輯後果自負：因為可能會編輯出故障來。
  4. 當使用者片語資料架內出現任何內容變動的時候，都會重新載入半衰記憶模組的內容。
  5. 半衰記憶模組會在觀測時確保辭典內的資料順序按照最後編輯順序做新舊排列（詳見源碼內文註解）。
  6. 半衰模組對輸入時的自動選字以及選字窗的內容排序的影響可以在偏好設定的辭典選項內藉由對應的勾選項來停用（打勾則為啟用），但半衰模組仍會持續運作。
- 功能：當新酷音的 symbols.dat 被放入威注音的使用者片語辭典目錄內的話，威注音的波浪符號鍵選單會轉而讀取這個檔案來作為波浪符號鍵選單的內容。這樣一來，使用者就可以自訂波浪符號選單的內容。目前先不對此做「簡體中文/繁體中文模式區分不同的 symbols.dat 檔案」的設計，因為功能實在太小眾，且：如果引入了該設計的話，反而會麻煩使用者給檔案重新命名。再加上威注音原本的波浪符號鍵選單的內容本來就很豐富。
...
- 研發：對 LMConsolidator 模組（以及其在使用者片語編輯器內的近親）某些處理細節做了小規模調整。
...
- 懶人包總結：組字區內可直接看到康熙字轉換／JIS 漢字轉換的結果；由於 Facebook 綁架 Tab 鍵，允許使用 Shift(+Command)+Space 直接就地輪替候選字；大量程式維護與 documentation 漢語重寫。
...
- 懶人包總結：選字窗新增頁碼顯示；新增對單個漢字候選字詞升頻或排除的選項；更智能的橫排/縱排選字窗判斷（依候選字字符總數）；鐵恨引擎更新至 v1.1.5、天權星引擎回退至 v1.1.8。
- [SP2] 新增新加坡漢語常用詞語。
...
- 選字窗現會在右側顯示當前的頁面編號。
- 在偏好設定內新增了一個選項，允許使用者對單個漢字的候選字詞升頻或排除。該功能不預設開啟，是因為考慮到該功能可能會導致單個漢字「頻率過高」打斷自動選詞的危險。如果真出現這種危險情況的話，編輯使用者片語檔案、將相關的行記錄移除即可。威注音反而鼓勵使用者們就常用漢字的排序提出調整建議（當且僅當某些常用漢字不在前十八個候選字內的情況下）。
...
- 懶人包總結：允許停用任何與各種輸入模式切換功能綁定的鍵盤熱鍵開關；新增 Ctrl+Alt+逗號/句號輸入雙書名號《》；可調整組字緩衝區最大容量（10-40 字）；選字風格回歸真正的微軟新注音式游標選字風格。
- 現允許停用任何與各種輸入模式切換功能綁定的鍵盤熱鍵開關。詳情請洽「偏好設定→鍵盤→鍵盤熱鍵」。因為 macOS 系統的 UserDefaults 管理特性的原因，做出這種修改之後需要一秒左右才能生效。
...
- 按鍵行為：現在，當 (Shift+)Tab 鍵的輪替對象被設定為候選字（而非選字窗的頁面）時，敲字時直接摁 (Shift+)Tab 會就地輪替候選字、而不出現選字窗。此時如果手動叫出選字窗的話，(Shift+)Tab 會像之前那樣在選字窗內輪替漢字。該設定不會在啟用逐字選字輸入模式時妨礙使用者輸入 Tab 縮進。
...
- 懶人包總結：緊急修正天權星引擎 v1.1.1 引入的「新輸入的低頻詞綁架高頻詞」故障。
- 緊急修正了天權星引擎 v1.1.1 版引入的一處故障。現在，威注音輸入法不會再出現之前威注音 1.6.0-1.6.1 版當中的「新輸入的低頻詞綁架高頻詞」的情況。
- 例行詞語更新。

## 1.6.1

- 懶人包總結：原廠詞庫格式更換為 plist，開機後首次啟動（或重啟）輸入法的等待時間縮短至約一秒；更換內建蜂鳴等音效為重新設計的高傳真音效；可啟用「按候選字詞的長度調整權重」。
...
- 懶人包總結：糾正幾處與 IMK 有關的互動行為、避免幽靈組字區；重新啟用酷音波浪鍵符號選單（在 Word 與包括內建終端機在內的各種終端機應用內正常使用）；遞交內容時不再放行任何 ASCII 不可列印字元。
...
- 懶人包總結：鐵恨注音並擊引擎更新至 v1.1.0——重新支援「漢語拼音」輸入、並新增國音二式／耶魯拼音／華羅拼音／通用拼音支援（皆以數字鍵標調）；SwiftUI 偏好設定鍵盤頁新增注音↔拼音快速切換按鈕。
...
- 懶人包總結：新增「拼音並擊模式」與「可自選以 Ctrl(+Alt)+CMD+Enter 輸出注音文還是拼音文」；修正 Delete 鍵幽靈組字區與 1.5.4 版起組字區中央編輯的錯亂；重新啟用 reverseWalk() 的進階功能。
...
- 懶人包總結：更換注音並擊引擎為鐵恨引擎（專案自此移除全部 Lukhnos Liu 的 C++ 源碼成分）；暫時移除漢語拼音輸入支援；修正偽精業鍵盤佈局檔案的一處映射錯誤。
...
- 懶人包總結：天權星引擎升級至 1.0.7、對 reverseWalk() 做功能拓展（span 跨度越長權重越大）；以 .split() 取代 .components()，核心語料載入時間由四秒縮短至一秒、峰值記憶體佔用由 115MB 降至 74MB。
...
- 懶人包總結：新增以 Ctrl+Alt+Command+Enter 直接輸出 W3C 標準的 HTML Ruby 漢字注音標示；修正 F1-F20 等按鍵導致輸入法崩潰的問題；記憶體佔用峰值由 200MB 降至 114MB（LMCoreEX 免析語言模組）。
...
- 懶人包總結：輸入法的語言處理引擎由 Gramambular 更換為純 Swift 的 Megrez 天權星引擎（KeyHandler 與語言模型管理器徹底 Swift 化）；核心語言模組資料按簡繁模式分別載入；半衰記憶模組功能暫時缺席。
...
- 從這個版本開始，半衰模組功能暫時從威注音缺席。一是因為用 Swift 重寫之後發現完全沒效果、也不知道該怎麼寫單元測試來找出毛病，二是因為這個功能會給某些使用者帶來額外的困擾：比如說剛剛用 SHIFT+方向鍵選字後摁 Enter 就地新添入的使用者片語可能會因為半衰模組的殘存記憶而無法立刻生效，等。雖然重啟一下輸入法就好，但還是會很麻煩。回頭等半衰模組功能修好之後，威注音會考慮在每次就地新增使用者片語的時候重設半衰模組的內容。

## 1.5.3

- 懶人包總結：集中調整原廠語彙庫的內容及頻率；規範游標前置/後置模式的稱謂（改用「macOS 內建注音風格」與「Windows 版奇摩輸入法風格」）；修復與游標前置模式有關的行為倒退。
...
- 懶人包總結：解決了源自上游引擎的「啟用游標前置模式時、游標在詞語中間也會叫出候選字」的 Bug；用語調整：「配列」→「排列」、「不要放屁」→「廉恥模式」、「康熙字」→「康熙正字」。
...
- 使用者片語編輯器的視窗的預設尺寸與最小尺寸擴增至 640x480。
...
- 懶人包總結：用 Swift 重寫了 KeyHandler 所有能重寫的部分（約 80%），藉此換取更快的輸入法響應速度；解決了手動重啟輸入法後不尊重當前簡繁模式、以及重啟後首個文字輸入失敗的問題。
- 用 Swift 重寫了 KeyHandler 所有能重寫的部分（約 80% 左右），順手解決了該模組內被 Objective-C++ 隱蔽的全部的類型安全問題，藉此換取了更快的輸入法響應速度、與更舒適的程式專案維護體驗。
- 調整了使用者片語編輯器的預設字號大小，使其更易於閱讀。
- 新增隱藏功能：允許使用者在開啟調試模式的情況下「摁著 Alt 鍵點輸入法的對應選單項目」以同時開啟簡繁體雙模式的使用者片語檔案。除此之外的場合，只會開啟當前簡/繁體模式對應的使用者片語檔案。
...
- 懶人包總結：macOS 11 開始藉由輸入法選單直接點開的偏好設定改為 SwiftUI 新版介面（摁 Alt 點選單可叫出舊版）；組字區不為空時摁漢字輸出轉換與語言模組模式轉換熱鍵會先遞交內容；並未放棄對 macOS 10.11 的支援。
...
- 使用者片語：解決了舊版偏好設定視窗在選擇使用者片語辭典目錄時「沒能對目前選中的目錄正確判定是否具有寫入權限」的設計缺陷。
...
- 檔案系統行為：如果輸入法發現使用者片語檔案路徑對應的對象不是資料夾而是檔案的話，不再套用上游的做法（就地移除），而是分情況：如果該路徑是原廠路徑的話，則對對象進行更名（而非移除）；如果該路徑是使用者指定過的路徑的話，則直接報錯。
...
- 研發：針對 ctlPrefWindow 舊版偏好設定視窗控制器與 mgrLangModel 語言模組管理器內的一些函數做了調整、以便準確核查指定的使用者片語檔案目錄是否有寫入權限。本次針對 macOS 11.x 及之後的 macOS 系統使用 SwiftUI 重寫的偏好設定視窗沒有這個問題。
- 研發：使用 Swift 將 mgrLangModel 語言模組管理器內所有可以用 Swift 改寫的部分全都用 Swift 進行了改寫。該過程中發現了一些藉由 Objective-C 無法曝露出來的類型安全問題，均順手解決。
- 研發：將 mgrLangModel 語言模組管理器內的「用以檢查具體的使用者片語檔案是否存在」的相關程式碼做了精簡處理。
...
- 懶人包總結：預設波浪鍵符號選單回歸至 1.2.x／小麥注音 2.2.2 的式樣、1.3.0-1.4.6 的「新酷音式波浪鍵選單」改以「Alt+波浪鍵」雪藏；全專案引入 Clang-Format 格式整理機制。
...
- 懶人包總結：新增摁 Alt 點開輸入法選單時的卸除輸入法選項；支援以終端指令卸除輸入法（「uninstall --all」可連同使用者偏好設定與使用者片語檔案一併清理）。
1. 新增功能：當摁著 Alt 鍵的時候點開輸入法選單的話，會出現允許卸除輸入法的選項。
2. 新增功能：以非 sudo 命令且以「uninstall」參數藉由終端運行輸入法的話，會卸除輸入法。在此基礎上如果將參數換成「uninstall --all」的話，則會連同使用者偏好設定與使用者片語檔案資料夾一同清理乾淨。
...
- 懶人包總結：允許使用者自行指定使用者片語檔案的存放目錄；解決了舊版 macOS 系統下無法顯示偏好設定與關於視窗的失誤。
1. 本次唯一的功能更新：允許使用者自行指定使用者片語檔案的存放目錄。如果該設定值為無效值的話，則會自動使用預設目錄。
...
5. 所有與使用者片語庫重載有關的命令均由 AppDelegate 挪至 IME 模組、使用單獨的函數來管理，且在 AppDelegate 當中直接調用該函數。
6. 規範了偏好設定內某些與鍵盤佈局有關的術語與說明描述，使其更易懂。
7. 解決了自威注音 1.4.1 / 1.4.2 版對 MainMenu.xib 失手引入的一處「在舊版 macOS 系統下無法顯示偏好設定與關於視窗」的失誤。

## 1.4.4 - LTSB

- 懶人包總結：本版僅確保在 macOS 12 Monterey 系統內可正常全功能執行（對 macOS 10.11.5 的相容支援已於 1.4.5b 補回）；移除上游繼承的「基於修飾鍵的進階鍵盤配置」功能；選詞後游標自動置於選中詞的前方。
...
- 懶人包總結：解決了「摁著修飾鍵點開某些 App 的 NSMenu 後無法正常輸入中文」的問題；針對動態注音（大千＆倚天）鍵盤佈局引入 Alt-Key 標點輸入設計。
該版本僅確保在 macOS 12 Monterey 系統內可正常全功能執行。對 macOS 10.11.5 的相容支援已經在 1.4.5b 補回。
...
- 懶人包總結：允許在就地新增語彙時摁「SHIFT+CMD+ENTER」將既有語彙塞入排除表（俗稱「就地刪詞」）；詞庫編譯腳本改用 Swift 重寫、修正大量語料格式錯誤；注音文輸入按鍵統一為「CTRL+CMD+ENTER」。
該版本僅確保在 macOS 12 Monterey 系統內可正常全功能執行。對 macOS 10.11.5 的相容支援已經在 1.4.5b 補回。
...
6. 使用者片語編輯器的格式整理腳本優化，減少了非必要的 RegEx 處理。
...
- 懶人包總結：本版廢版處理（誤將小麥注音的兩處始祖設計認為是 Bug 根源、導致 F1-F12 鍵盤對組字區的不當操作），並重新 Rebase 至 1.4.2 版。
...
- 懶人包總結：解決了以 SHIFT 鍵往組字區輸入字母時的判定問題（不再出現「試圖敲漢字卻敲出大寫字母」）；新增使用者繪文字語言模組；圖示重新設計；單引號鍵依台灣使用者習慣預設輸出全形頓號。
...
3. 新增 UserSymbolLM 使用者繪文字語言模組，隨使用者片語自動載入。這樣會方便那些想自訂國旗類繪文字的人群們自行滿足個人需求，畢竟威注音是不會在原廠語料庫內放入這一類資料的。
4. 輸入法與威注音語彙編輯器的「關於」視窗內新增了前往威注音 wiki 的按鈕。
5. 解決了使用者片語檔案自動整理模組會在整理格式時重複插入格式整理標頭的問題。
...
- 懶人包總結：修正切換簡繁體中文模式後不會自動同步「全字庫」「繪文字」等子語言模組開關狀態的問題；Caps Lock 英數模式現在會切換鍵盤佈局至 Apple ABC 佈局；注音文輸入熱鍵統一為 Ctrl+Enter。
...
- 使用者片語：重新引入 OpenCC 實現了一個新功能：在某個簡繁體模式內使用「摁 SHIFT+方向鍵選擇範圍、敲 ENTER 確認」的方式就地新增使用者片語時，會自動將藉由 OpenCC 轉換過的結果塞入另一個簡繁體模式下的使用者片語檔案內、且附以特殊標記（方便人工排查轉換結果是否準確）。
- 使用者片語編輯器：糾正一處本地化翻譯錯誤（「轉換微小寫」→「轉換為小寫」）。
- 使用者片語編輯器：糾正了正則轉換功能當中的 NSRange 範圍選定錯誤、且調整了正則轉換順序。
- 語料庫：新增大中華地區的省市名稱記錄。
- 語料庫：新增了一些繪文字內容。
...
- 為了維護威注音的合法性原則，今後威注音並不計畫加入任何國旗類 Emoji。有相關需求者，請自行增補各自的使用者片語檔案。

## 1.3.7 OSR3

- 懶人包總結：糾正了「無法叫出波浪符號按鍵」與「想輸入半形減號卻變成別的符號」的問題（改用 KeyCode 判定）；支援日語拗音輸入；允許對調 Shift+Space 與 Space 鍵在選字窗內的行為。
...
- 使用者片語編輯器：允許將漢語拼音「cei」轉換「ㄘㄟ」，聲調不限。
- 雜項：現允許在威注音語彙編輯器以外的程式當中使用 Ctrl+CMD+Enter 將當前組字緩衝區的內容以注音文的形式輸出（且在這種模式下輸出的注音不以減號區隔、而是以半形空格區隔每個漢字的讀音）。
- 雜項：針對使用者片語編輯器與使用者核心語料庫編譯腳本的 Regex 處理部分做了效能優化處理。
- 雜項：使用者片語編輯器的軟體封包名稱現已被正確本地化。
...
- 懶人包總結：允許使用者將想要加入使用者語匯的內容真正地提權至頂；修正自動更新檢查功能（會正確彈窗提醒更新）；重新支援最多十字長度的候選字。
...
- 懶人包總結：加入摁 TAB 呼出選字窗、以及在選字窗內以 TAB/Shift+TAB 翻頁與輪選候選字的功能；PageUp/PageDn 在單頁時兼任 Home/End 鍵；引入偵錯模式。
...
- 移除了針對使用者片語檔案的內容排序功能，因為 C++ 的排序行為不可控。<br />// 筆畫排序、Unicode 碼位排序、拼音排序、注音排序……眾口難調。
- 加入了摁 TAB 呼出選字窗的功能。
...
- 懶人包總結：重新允許使用者添入重複語彙（於對應詞條行尾留下特殊標記）；補錄兩岸漢字異讀資料當中的全部陸規審音；可利用注音符號拼讀的方式輸入日語假名。
- 威注音重新允許使用者添入重複語彙、但會在使用者片語檔案內的對應詞條的行尾留下特殊的標記。<br />// 威注音 1.3.0 版受上游影響，在使用者就地添加自訂語彙時，不允許添入已經存在於當前語料模型內的語彙（無論是原廠語彙庫還是使用者片語庫）。然而，使用者可能會對原廠語彙庫內的詞語的排序不滿。這也是雅虎奇摩輸入法的痛處之一。考慮到使用者片語庫可以用來覆寫原廠語彙庫內的優先權級，且威注音有語彙檔案格式整理系統（來專門負責自動將使用者片語內的重複條目清理乾淨），所以才有了這次更動。
- 修復了簡體中文輸入模式下在就地新增使用者片語時無法正確地進行重複判定的問題。<br />// 該問題自 v1.3.0 引入，原因是當時忘記對來自上游的該功能做這種處理，導致在簡體中文輸入模式下做重複判定時拿了繁體中文語料模型作為參考對象。
...
- 懶人包總結：緊急修正「繁體中文模式下在同時關掉康熙與 JIS 轉換時無法敲字」與選字窗迴圈翻頁頁碼統計錯誤；淘汰 OpenCC 與 SwiftyOpenCC、改用自製的康熙正字／JIS 新字體漢字轉換模組；選字窗支援迴圈翻頁。
...
- 懶人包總結：緊急解決了輸入法無法在終端機內敲字的問題；新增偽精業鍵盤佈局；新增 Apple 注音鍵盤佈局支援（達成對 macOS 內建螢幕小鍵盤的支援）；收錄《國語一字多音審訂表》研究當中的合理民間讀音。
...
- 懶人包總結：波浪符號選單內的分類名稱針對不同語系介面完成本地化；新增神通注音鍵盤佈局；針對「SHIFT+方向鍵選字時的選區容量計算失誤」引入保守治療方案。
...
- 懶人包總結：引入威注音語彙編輯器（可自動糾正格式、將漢語拼音二式轉換成注音）；新增使用者自訂聯想詞功能；引入波浪鍵雙層符號選單（可輸入貨幣符號、箭頭等）；可自訂語彙長度由 6 字擴增至 10 字。
- 威注音語彙編輯器。之前 1.2.x 版會自動對使用者片語檔案進行格式整理，但這樣卻會導致與系統內建的 TextEdit 編輯器的行為衝突。雖然使用者可以強行存檔，但畢竟這種情況還是會令使用者困惑。於是威注音在這次更新當中引入了一款簡易的語彙編輯器，不但可以在存檔讀檔過程中自動糾正格式、還會自動將漢語拼音二式轉換成注音。此外，經過威注音語彙編輯器與威注音輸入法統整過格式的檔案，都會在第一行出現專有的辨識標頭、以令威注音輸入法本體在讀到該標識時不再自動統整檔案格式（但仍會檢查檔案結尾是否缺少換行符，此乃 unix 規範）。
...
- 懶人包總結：簡體中文輸入模式的選單圖示顏色變動；簡繁體中文輸入模式的輸入法選單列標籤更名為「威注音-簡/繁」；修正 info.plist 一處語言標籤錯誤。
...
- 懶人包總結：威注音首發版：簡繁體中文詞庫彼此分離且徹底重塑（詞頻數據來自 NAER）；支援通用規範漢字表全部漢字的單獨輸入；讀取使用者自訂語匯檔案時自動統整格式、不擅自連網；提供模擬逐字選字（ㄅ半）輸入的功能。
