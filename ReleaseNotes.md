...
- 修復：[所有發行版] 解決了在就地加詞時可能會導致輸入法卡頓一兩秒的故障。該故障往往在使用者辭典檔案記錄超過五千筆的時候發生。
...
- 體驗：[所有發行版] 允許在重設或指定新的使用者辭典目錄時將舊位置的資料併入新位置。
...
- 修復：[所有發行版] 針對 CIN2 磁帶模式追加了新的保底機制。當使用者將磁帶檔案放在由 iCloud Drive 代管的 Documents / Desktop 等鏡照目錄、導致沙箱書籤在重新開機或系統更新後失效時，輸入法現在會自動回退讀取保存在 App Support 目錄內的快取副本，以提升磁帶模式的可用性與可靠性；若使用者在指定磁帶檔案或使用者辭典目錄時實際挑中的是 Symbolic Link，輸入法也會先解碼到來源位置再保存設定。
- 體驗：[所有發行版] 現在若使用者將 CIN2 磁帶檔案或使用者辭典目錄指定到 iCloud Drive 代管的 Documents / Desktop，輸入法會提供更精確的提示訊息，提醒使用者改用本機目錄；若不確定系統的 iCloud Drive 鏡照資料夾設定，也可依照提示向 Apple Support 洽詢。
- 體驗：[所有發行版] 修訂了與卸除輸入法有關的介面說明，方便使用者更快理解為何需要改採手動卸除。
- 辭典：[所有發行版] 例行語彙資料更新。

## 4.3.6

- 修復：[所有發行版] 緊急修復了唯音輸入法的沙箱書籤管理系統與 macOS 26.4.1 的 iCloud Drive 的相容性故障。該故障會在使用者將使用者片語目錄設在 iCloud Drive 時發生、讓輸入法每次嘗試存取使用者辭典目錄時都會連輸入法加上「正在接收文字輸入的軟體」一同僵死數秒。
...
- 修復：[所有發行版] 讓使用者辭典模組就檔案讀寫方面使用統一的 Queue 來完成任務。這解決了兩個問題：「將 iCloud Drive 設為使用者辭典目錄後，可能會隨著時間推移而產生大量的重複檔案」以及「在新的客體（接收文字輸入的軟體）內開啟新的打字會話副本時，偶爾會梗死幾秒鐘」。
- 研發：[主流發行版] 對整個專案做了大手術，改用 Swift 6.2 且做了與此有關的多工處理技術迭代處理，以允許該輸入法的打字模組在 Windows / Linux 系統下建置、開發。利用了 Swift 6.2 的 Default Isolation 特性，將包括打字會話在內的所有內容都預設釘死在 MainActor 上。某些特種後檯任務（比如使用者辭典檔案讀寫）另論。註：Windows 版唯音輸入法的正式開發仍需等待 Swift 接下來在 Windows 平台的對 Static Linking Against Foundation 特性的支援，且需要相當的資金捐助以支撐開發者的脫產全勤開發之開銷，還需要專門針對 Windows 平台的一些特性做精細的交互設計與相容處理，總之絕非彈指一瞬的事情。
...
- 體驗：[所有發行版] 在「開發道場」內新增了一個功能，允許使用者暫時停止使用者語彙辭典對組字引擎的任何影響。開發者可能會讓使用者啟用該功能、便於讓使用者迅速評估其所遇到的組句故障的癥結是否位於使用者辭典內部。
