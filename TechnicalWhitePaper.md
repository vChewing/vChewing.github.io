...
自 2022 年三月底開始，威注音嘗試繼續之前上游的 Swift 化計畫所未完成的內容：用 Swift 將整個專案的 ObjC(++) 與 C++ 的部分用 Swift 徹底重寫。先是重寫了按鍵調度模組，之後是組字引擎「Megrez」，最後威注音又有了自己原創的功能更豐富的聲韻並擊引擎「（齊）鐵恨 Tekkon」。此外，威注音當時嘗試用 C# 語言重寫這些模組，但因為 Windows「沒有與 InputMethodKit 一樣方便的輸入法開發套裝體驗」而只能作罷，相關倉庫荒廢至今。在此之後，Megrez 引擎做過一輪技術升級。

然後就是到了八月初，威注音實現了 IMK 矩陣選字窗的支援。由於上游有人在八月底對於威注音輸入法的 Shift 按鍵判定行為有關的不實言論，威注音專案啟動了「D 計畫」，在一個月內將輸入法本體的所有「直接從上游繼承來的 Swift 模組」全部淘汰，且用 SwiftUI 完成了威注音自家的全新的田所選字窗：這套選字窗同時實現了（類似 IMK 選字窗的）橫排矩陣佈局與（類似微軟新注音的）縱排矩陣佈局。而上游那位某人還在那裡止步於一月底「有種衝動想把小麥注音裡頭的 UI 都換成 SwiftUI」那種衝動而已。

然而，仍舊會有諸如 BrLi 之流拼盡一己全力混淆視聽。於是，就有了筆者這篇技術白皮書。

## 總括概述

###  基礎大模組與處理單元

差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.5) | 補記
-- | -- | -- | --
按鍵調度模組 (KeyHandler) | Objective-C++ | 以 Swift 重寫 |  
按鍵訊號承載單位 | KeyHandlerInput | NSEvent及其私有功能拓展 | 威注音利用 NSEvent 的 characters ignoring modifiers 參數塞入狀態上下文情境描述用中繼資料
...
注音並擊引擎 | OVMandarin (C++)<br />by Lukhnos Liu | Tekkon (Swift)<br />支援更多注音排列與拼音種類 |  
詞庫管理模組 | Objective-C++ & C++<br />ParselessLM<br />採 TXT 格式的原廠辭典 | Swift<br />採 Plist 格式的原廠辭典<br />使用者辭典採 txt 格式 | 威注音在這方面有著記憶體佔用率方面的劣勢，記憶體佔用率約為 150MB 以內。
態械引擎 | 多型別狀態策略設計模式無專有內部資料型別 | 單結構狀態策略設計模式有專有內部資料型別 | 威注音的設計更易於維護

###  選字窗

差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.5) | 補記
-- | -- | -- | --
IMK 選字窗支援 | 無 | 有，依賴狀態管理＋ bridging header 強制曝露內部 API | 僅橫排選字窗在 macOS 10.14 開始支援捲動矩陣佈局
輸入法內建選字窗 | Voltaire MK2 (Swift Cocoa) | Tadokoro 田所 (SwiftUI)以取代 Voltaire MK3 | 田所選字窗僅支援 macOS 10.15 開始的系統版本
...
針對簡繁體輸入模式以對應的系統介面字型顯示候選字 | 無 | 僅限田所選字窗（需在偏好設定內的開發道場當中手動啟用） | 田所選字窗對此要求至少 macOS 12
...
差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.5) | 補記
-- | -- | -- | --
支援注音排列種類 | 大千傳統、倚天傳統、倚天26、許氏、IBM | 大千傳統、倚天傳統、IBM、神通、(偽)精業、倚天26、酷音大千 26 、許氏、星光 | 下述排列為動態注音排列：倚天26、酷音大千 26 、許氏、星光
支援拼音輸入種類 | 僅一種模式，缺乏介紹資料 | 漢語拼音、國音二式、華羅拼音、耶魯拼音、通用拼音 |  
...
Emoji 模式 | 僅原廠 Emoji，無開關自訂 Emoji 到使用者辭典內的話，會與漢字詞搶頻 | 有專門的使用者 Emoji 辭典檔案，有開關 |  
...
加詞刪詞範圍運算管理 | 僅位於態械內，以 UTF16 為範圍長度單位，需 NSStringUtils。 | 態械、組字引擎、按鍵調度引擎同時協作，以 UTF8 為範圍長度單位，不需要 NSStringUtils。 | 威注音的做法更精確，也只有這樣才能實現對字詞數與讀音數不等的情況的支援
...
內文組字區防洩密 | 無 | 有（需要在客體管理器內針對具體的客體 App 啟用浮動組字窗） | 會有 App 或網站提前獲取內文組字區的內容給遠端伺服器、來遠端預判使用者的下一步操作。
讀取網路資料 | 僅在新版軟體檢查時才會聯網，且該功能自動啟用、會在安裝完畢後首次執行時主動聯網 | 僅在新版軟體檢查時才會聯網，需使用者手動啟用新版軟體檢查之功能 |  
將本地資料上傳 | 無 | 無 | 威注音另有與此有關的 Sandbox 約束
...
其他漢字支援 | 無 | 繁體轉 JIS 漢字繁體轉康熙 | 轉換準確度有限
組字區與選字窗內的漢字反應當前的簡繁模式或 JIS / 康熙轉換模式 | 否 | 是 |  
審音 | 僅台澎金馬民間讀音 | 兩岸審音與台澎金馬常用讀音均兼顧 |  
金融數字轉換模式 | 無 | 有 | 銀行等場合會用到

###  其他功能

差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.5) | 補記
-- | -- | -- | --
Steam 支援 | 無 | 有（需要在客體管理器內針對具體的客體 App 啟用浮動組字窗） | 對其他「不認真遵守 IMKTextInput 協定」的軟體也適用
使用者辭典格式主動整理 | 無，只會在就地加詞時套用檔案 EOF 檢查修正 | 有，會主動整理 |  
使用者辭典目錄狀況監測 | 有：FSEventStream | 有：Dispatch Source File System Object (DSFSO) | DSFSO 無法監測 App 自身對目錄內的檔案的修改
偏好設定視窗 | 僅 XIB | XIB 給舊版系統用，SwiftUI 給新版系統用 |  
敲字讀音錯誤提示 | 僅系統蜂鳴生效 | 有專門的SFX | 威注音的廉恥模式的開關會影響 SFX
ㄅ半模式支援 | 單獨的輸入法模式副本 | 輸入法功能選單內切換 | 威注音的聯想詞模式可自訂詞庫
自訂語彙長度限制 | 六個字 | 十個字 |  
...
輪替候選字 | 僅 (Shift+)Tab | Shift(+Alt)+Space<br />Alt+上下<br />(Shift+)Tab | 縱排輸入時為 Alt+左右
...
日期時間便捷輸入 | 不支援 | 支援 |  
Emacs Key | 在按鍵調度模組內部處理 | 在水源入口處理：發現 Emacs Key 就直接將原始 NSEvent 換成翻譯後的 NSEvent |  
...
此外，考慮到有使用者在濫用這個功能的情況下出現的記憶體洩漏、CPU 高佔用率的問題，威注音輸入法在 2.8.0 SP3 當中對此做了專門的修正：最多只會殘留四條近期通知。
...
小麥注音的 KeyHanderInput 是把 NSEvent 讀取且翻譯成這麼一個新的按鍵訊號載體單位類型。該組件在威注音當中被更名為 InputSignal，但與 InputSignalProtocol 是兩碼事情：前者遵循後者這個協議所規定的規範，而 KeyHandler 按鍵調度模組當中的函式可以處理任何符合該規範的物件。
...
更甚者，這次的組字引擎也做了升級、可以單獨處理要在標記模式下使用的標記游標。這樣一來，KeyHandler 的任務輕鬆了不少，**且威注音終於在原理上實現了在就地加詞刪詞時對詞音不等長的情形的正確應對處理**。這些都使得小麥注音 2.x 的另一個模組「NSStringUtils」不再重要：<u>每次在標記模式當中修改標記範圍時，對新的標記範圍的運算處理都是在組字引擎內產生的、然後被 KeyHandler 翻譯出來、變成更新過的標記狀態當中的資料的一部分</u>。**小麥注音 mac 版受制於 Swift 與 C++ 的生殖隔離，僅依賴 Objective-C++ 對兩者的橋接的話，很難做到這些**。
