...
以下列出唯音自自立分支後至 v4.4.0 之關鍵里程碑（日期以實際開發活動階段為準，非全部為正式發佈日）:
...
差異項 | 小麥注音 (3.0) | 唯音 (4.4.0) | 補記
-- | -- | -- | --
支援作業系統範圍 | macOS 11 起算 | 主流發行版 macOS 13 起算<br />Aqua 紀念版 macOS 10.9 起算 | macOS 10.9 是 Swift 5.9 能支援的最早的系統版本。Swift 6 開始將不再支援 macOS 10.9 等任何需要 libArcLite 的系統版本。
...
組字引擎 | Gramambular 2 (C++)<br />by Lukhnos Liu<br />2026 年 1 月底為止採 DAG-Relax 組字算法；於次月被 Chiahong 換成 DAG-DP。 | Homa (Swift)<br />by Shiki Suen<br />2025 年 10 月改採 DAG-DP 算法（前身 Megrez 3.9.x 曾用 Dijkstra）；2026 年 4 月升級為 Homa，內建候選輪替、上下文鞏固與感知觀測 API。 | Homa 擁有包括對就地加詞功能要用到的標記游標的管理等獨特功能。DAG-DP 算法無須 Vertex Topological 排序。
...
Unicode 萬國碼資訊顯示 | 無（Voltaire 選字窗不支援） | 有<br />僅限田所選字窗 | 該功能自 3.8.1 版開始實裝。
候選字朗讀 | ✅（2.9.3+）使用 macOS 系統內建的 SQLite 輔助功能資料庫；僅與 VoiceOver 聯動 | ✅（4.1.0+ 可設定）查詢使用者自訂關聯詞語資料庫 | 唯音沿用了自身本來就有的語音朗讀模組
| 選字窗畫面控頻/刪詞 | 自 v2.9.3 開始有在選字窗狀態下引入基於熱鍵的升頻交互畫面，但不能降頻 | 右鍵選單控頻刪詞<br />僅限田所選字窗（註：自 v4.2.0 起，對單個漢字的升頻/降頻/排除操作之開關已移至「偏好設定 → 開發道場」，預設關閉；當未啟用時，使用者片語辭典內的單漢字升降頻覆寫不再生效。此前啟用過該選項的使用者在升級到該版本時，該選項會被強制取消勾選；使用者可在升級後重新啟用，但後果自負。） | 不支援臨時漂浮客體，<br />比如 Spotlight 與 NSMenu 等。
...
用聲調鍵複寫游標後方的漢字的聲調 | 至 2.9.4 為止的版本不支援 | 支援 |
允許以聲調鍵開頭敲字 | 支援 | 支援 |
...
偏好設定視窗 | 僅 XIB | 採用唯音自己訂製的類似 SwiftUI 的 DSL 語言來通過 AppKit 構建視窗。<br />另外，主流發行版也有採 SwiftUI 實作另一版偏好設定視窗、且預設啟用 SwiftUI 偏好設定視窗。 |  
...
W3C Ruby 注音讀音標記 | mac 版無，linux 版有；不支援教科書聲調寫法 | 有，且支援教科書聲調寫法 | 唯音也可輸出漢語拼音的 Ruby 標記
