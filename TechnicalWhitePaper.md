...
差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.8) | 補記
-- | -- | -- | --
輸入調度模組 | KeyHandler (Objective-C++) | InputHandler (Swift) |  
...
差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.8) | 補記
-- | -- | -- | --
IMK 選字窗支援 | 無 | 有，依賴狀態管理＋ bridging header 強制曝露內部 API | 橫排選字窗在 macOS 10.13 系統下不支援矩陣捲動佈局，因為該版系統為止的 IMK 矩陣捲動佈局「沒有選字鍵的概念與設計」。
...
加詞刪詞範圍運算管理 | 僅位於態械內，以 UTF16 為範圍長度單位，需 NSStringUtils。 | 態械、組字引擎、 同時協作，以 UTF8 為範圍長度單位，不需要 NSStringUtils。 | 威注音的做法更精確，也只有這樣才能實現對字詞數與讀音數不等的情況的支援
...
差異項 | 小麥注音 (2.4.1) | 威注音 (2.8.8) | 補記
-- | -- | -- | --
Valve Steam 支援 | 無 | 有（需要在客體管理器內針對具體的客體 App 啟用浮動組字窗） | 對其他「不認真遵守 IMKTextInput 協定」的軟體也適用，比如 VSCode 與 Naver Line 等。
...
日期時間便捷輸入 | 不支援 | 支援 |  
Emacs Key | 在輸入調度模組內部處理 | 在水源入口處理：發現 Emacs Key 就直接將原始 NSEvent 換成翻譯後的 NSEvent |  
