...
詞庫管理模組 | Objective-C++ & C++<br />ParselessLM<br />採 TXT 格式的原廠辭典 | Swift<br />採 VanguardTrie.TextMapTrie 格式的原廠辭典<br />使用者辭典採 txt 格式 | 
...
| 選字窗畫面控頻/刪詞 | 自 v2.9.3 開始有在選字窗狀態下引入基於熱鍵的升頻交互畫面，但不能降頻 | 右鍵選單控頻刪詞<br />僅限田所選字窗（註：自 v4.2.0 起，對單個漢字的升頻/降頻/排除操作之開關已移至「偏好設定 → 開發道場」，預設關閉；當未啟用時，使用者辭典內的單漢字升降頻覆寫不再生效。此前啟用過該選項的使用者在升級到該版本時，該選項會被強制取消勾選；使用者可在升級後重新啟用，但後果自負。） | 不支援臨時漂浮客體，<br />比如 Spotlight 與 NSMenu 等。
...
差異項 | 小麥注音 (3.0) | 唯音 (4.4.0) | 補記
-- | -- | -- | --
就地加詞刪詞功能 | 僅就地加詞 | 就地加詞/刪詞/升頻/降頻（註：自 v4.2.0 起，對單漢字的升權/降頻/排除操作之開關已被移至「偏好設定 → 開發道場」，預設為關閉；當未啟用時，使用者辭典內的單漢字升降頻覆寫不再生效。） |  
...
唯音 v2.5.0 更換了新的 FolderMonitor 監視模組，利用了 DispatchSourceFileSystemObject。這個方法不會偵測由輸入法本體對目錄做出的修改，所以筆者又在 LMMgr (Language Model Manager) 當中對「使用者手動加詞」的情況又補上了手動重新載入使用者語彙的動作。這也解決了迄今為止 FSEventStreamHelper 與「唯音的使用者辭典格式自動整理模組」彼此的行為衝突問題。
