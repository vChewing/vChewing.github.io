...
候選字朗讀 | 有（自 v2.9.3 開始）<br />與 VoiceOver 聯動 | 有（自 4.1.0 版開始）<br />僅限田所選字窗 | 小麥注音會在 VoiceOver 啟用時自動朗讀當前選中的候選字，並使用關聯詞語資料庫及 macOS 輔助功能資料庫來說明同音候選字（例：「中國地方的中」）。威注音可設為始終開啟、隨 VoiceOver 啟用或停用、或始終關閉（預設）。若有同音候選，系統會查詢可由使用者自訂的關聯詞語資料庫以嘗試說明各字。
資料截取日期 | 2025-11-11 | 2025-11-11 | （以下條目為差異項）
...
羅馬數字輸入 | 有（自 v2.10.0 開始）<br />Ctrl+\\ 啟動 | 有（自 4.1.0 版開始）<br />Shift+Cmd+R 啟動 | 小麥注音支援三種轉換風格：字母（2025→MMXXV）、全形大寫（2025→ⅯⅯⅩⅩⅤ）、全形小寫（2025→ⅿⅿⅹⅹⅴ）。威注音支援數字範圍 1-3999，提供大寫（Ⅰ、Ⅱ、Ⅲ）與小寫（ⅰ、ⅱ、ⅲ）兩種全形風格。
