...
然後就是到了八月初，威注音實現了 IMK 矩陣選字窗的支援。由於上游有人在八月底對於威注音輸入法的 Shift 按鍵判定行為有關的不實言論，威注音專案啟動了「D 計畫」，在一個月內將輸入法本體的所有「直接從上游繼承來的 Swift 模組」全部淘汰，且用 SwiftUI 完成了威注音自家的全新的田所選字窗：這套選字窗同時實現了（類似 IMK 選字窗的）橫排矩陣佈局與（類似微軟新注音的）縱排矩陣佈局。

然而，仍舊會有少數人士拼盡一己全力混淆視聽。於是，就有了筆者這篇技術白皮書。

## 總括概述

###  基礎大模組與處理單元

差異項 | 小麥注音 (2.4.1) | 威注音 (3.4.8) | 補記
-- | -- | -- | --
...
詞庫管理模組 | Objective-C++ & C++<br />ParselessLM<br />採 TXT 格式的原廠辭典 | Swift<br />採 Plist 格式的原廠辭典<br />使用者辭典採 txt 格式 | 威注音在這方面有著記憶體佔用率方面的劣勢，記憶體佔用率約為 150-190MB 以內。<br />如果僅使用簡體/繁體單一模式的話，記憶體佔用率會有明顯減少。
...
輸入法內建選字窗 | Voltaire MK2 (Swift Cocoa) | Tadokoro 田所 (SwiftUI/Cocoa)以取代 Voltaire MK3 | 田所選字窗的 Swift UI 分支僅支援 macOS 10.15 開始的系統版本
