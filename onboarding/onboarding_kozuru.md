...
小鶴音形雖然不是純字根輸入法（而是採前兩碼雙拼＋後兩碼字根的方式），但仍屬於唯音輸入法的磁帶模式的支援範圍內。
...
> ⚠️注意：
> 1. **請務必確保任何以 # 開頭的註釋行都不可以有半形空格與 Tab 符號的存在**，否則可能會導致唯音將註釋行解析成垃圾資料來讀入。
> 2. 小鶴音型的 CIN 磁帶受著作權保護，傳播之前請確認原作者的與此有關的要求。
