...
該頁面提供的方法理論上適用於所有版本的唯音輸入法。

如果您使用的是 2.2.0 為止的版本的話，要卸除唯音，請在確保當前安裝為完整安裝之後、摁 Option 鍵的同時點擊輸入法選單圖示，此時會在選單內看到「卸除唯音…」。

> 自 4.1.4 版開始，唯音輸入法收緊了自身的 Sandbox 策略。帶來的副作用就是自己無權卸除自己了。但這個功能仍舊會曬出與此有關的使用者檔案供使用者自行決定移除與否。 **剩下的內容請洽下文「我該手動移除哪些檔案」**。

自唯音輸入法 2.3.0 開始，對**因失誤而安裝到當前使用者目錄之外**的唯音輸入法的卸除，得採取該頁面所示的步驟。<br />
或者，如果你很懂終端操作的話，輸入法的 App Bundle 內也有用以卸除唯音的終端腳本可用。<br />
**這一切都是因為 macOS 內建的 App Sandbox 機制在預設的情況下不允許符合該機制的應用程式自己刪除自己**，才會這樣麻煩。<br />
唯音輸入法自 4.1.4 版起、因自身收緊的 Sandbox 策略而已無法自行移除自身套件；執行「卸除唯音…」時，唯音只會在 Finder 內打開相關檔案位置、請您自行移除（詳見下文「我該手動移除哪些檔案」）。<br />
如果您看不懂本文瑣示之操作的話，請持該網頁[向 Apple Support 尋求人工幫助](https://support.apple.com/zh-tw)、讓他們的專員幫您移除唯音。

> 為什麼要啟用 Sandbox 呢？這是**為了徹底粉碎某些「有損唯音輸入法程式清譽」的謠言**。唯音輸入法從第一版開始就是極度尊重使用者私隱的，為此甚至都不預設啟用自動版本更新檢查。唯音輸入法的 Shift 中英文開關也不會用到系統全局鍵盤事件監聽。各位只需要相信這一個事實就可以了：「**自 macOS 10.15 開始，是可以用 NSEvent 來判定 Shift 是否有摁過的**」。
