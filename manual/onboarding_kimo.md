...
從唯音輸入法 4.1.6 版開始，只需要在「輸入法選單」->「唯音輸入法偏好設定」->「辭典設定」裡面找到「匯入奇摩輸入法自訂詞資料庫」就可以匯入奇摩輸入法的使用者自訂詞資料。

- 如果您點的是「直接匯入」的話，會直接從當前電腦上的奇摩輸入法讀入使用者自訂詞資料。這並不要求奇摩輸入法必須在您當前電腦系統版本下的正常運作。
- 如果您點的是「…」按鈕、或者是「請將檔案拽到這邊」這種 Drag-and-drop-to 按鈕的話，則可以自行指定 SmartMandarinUserData.db 檔案的位置。

---

> 下文已過期，僅適用於「唯音輸入法 4.1.5 為止的版本」與「威注音輸入法 4.1.2 為止的版本」，過期時間戳是 2025 年 12 月 9 日。
>
> 唯音輸入法 4.1.6 版起已經支援對奇摩輸入法的 SmartMandarinUserData.db 的直接讀取、不再需要 NSConnection 跨程通訊。本文的方法仍有效，只是不再是最有效的方法。
