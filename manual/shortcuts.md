...
- 註4：唯音不會對數字小鍵盤攔截響應（小鍵盤 Enter 鍵除外），除非有選字窗出現、且有數字小鍵盤內能輸入的字符被設為選字鍵。
- 註5：本文下文出現的所有「數字鍵」均指主鍵盤區域的數字鍵。
- 註6：本文用語「（內文）組字區」代指「inline composition buffer」，在微軟術語體系當中也被稱作「pre-edit area」。
- 註7：本文用語「組音區」是指由鐵恨注拼引擎操縱的注音組音區。組音區會在使用者敲注音時出現在內文組字區內。輸入法自組音區取得讀音，在驗證其具有對應的辭典記錄之後，將其插入至組字引擎。整個內文組字區相當於組字引擎與注拼引擎的統合外在表現。
- 註8：為了保證表達與討論的有效性、減少歧義，本文在討論前後關係時，只用「往前/往後」「前方/後方」，且僅以文字書寫方向為這些用語的方向參照。這與 Zonble 的「Before-After」前後術語邏輯體系不同：唯音在使用英文術語的情況下只用「Front / Rear (Back, Tail)」、「Forward / Backward」。
...
| ⌃⇧⌘ K | 輪替漢字轉換（當代繁體 / 康熙 / JIS；僅對繁體中文輸出模式有效） |
| ⌃⇧⌘ O | 關聯詞語模式（就是ㄅ半的關聯詞語） |
| ⌃⇧⌘ J | 拼音 / 注音打字模式切換 |
| ⌃⇧⌘ L | CNS+GBEX 模式（不另外安裝全字庫／GBEX 字型的話，某些候選字會顯示成方塊） |
| ⌃⇧⌘ H | 半形標點模式（程式蝨專用） |
...
| ⌥→<br />⌥← | 令組字區游標向前移動一個詞語節點<br />令組字區游標向後移動一個詞語節點 | 組字區有內容時才會生效；<br />橫排輸入時為 ⌥→／⌥←，縱排輸入時為 ⌥↓／⌥↑。 |
| JIS英數鍵 | 中英文輸入模式切換 | 只有 JIS 日語鍵盤才有這個按鍵。 |
| ⇧↑<br />⇧↓ | 以固定順序就地輪替候選字（下一個候選字）<br />以固定順序就地輪替候選字（上一個候選字）| 組字區有內容時才會生效。 |
| ⌃⌘]<br />⌃⌘[ | 以固定順序就地輪替候選字（下一個候選字）<br />以固定順序就地輪替候選字（上一個候選字）| 組字區有內容時才會生效。 |
| ☰<br />⇧☰ | 以固定順序就地輪替候選字（下一個候選字）<br />以固定順序就地輪替候選字（上一個候選字）| 組字區有內容時才會生效。 |
| ⌥↑<br />⌥↓ | 以固定順序就地輪替候選字（下一個候選字）<br />以固定順序就地輪替候選字（上一個候選字） | 組字區有內容時才會生效；<br />橫排輸入時為 ⌥↑／⌥↓，縱排輸入時為 ⌥←／⌥→。 |
...
| ⌥⌃→<br />⌥⌃← | 迅速將組字區內的游標挪到組字區最前方<br />迅速將組字區內的游標挪到組字區最後方 | 組字區有內容時才會生效；<br />橫排輸入時為 ⌥⌃→／⌥⌃←，縱排輸入時為 ⌥⌃↓／⌥⌃↑。 |
...
| ⏎ | 將當前高亮標記了的詞音配對新增至使用者片語辭典，<br />如果已經是既有詞音配對的話、則會提升其權重。 | 輸入法處於標記模式時才會生效。 |
| ⇧⌘⏎ | 將高亮標記了的當前在庫的詞音配對的權重<br />壓低至 -114.514。 | 輸入法處於標記模式時才會生效；<br />相關記錄存於使用者片語檔案當中；<br />對使用者關聯詞語無法生效。 |
...
| ⌃⌥⌘⏎ | 依偏好設定「行為設定」頁的「Command+Option+Ctrl+Enter:」選項遞交內容：<br />預設為遞交括弧註解標記，可改為 HTML Ruby 註解標記、國語點字／國通盲文（含 ASCII 版）、ButKo BPMFVS 注音標記等。 | 組字區有內容時才會生效；<br />選用點字／盲文格式時，磁帶模式下一律遞交括弧註解標記；<br />同時摁 Shift 時只遞交讀音字串。 |
...
| ～ | 分類波浪符號選單（類似新酷音），<br />可在使用者片語辭典目錄內寫 symbols.dat 自訂內容。 | JIS 鍵盤注意：<br />得用右 Shift 左側的「_」鍵取代「～」鍵。 |
...
| 右⇧<br />左⇧ | 切換英數輸入模式的開關。 | 左⇧與右⇧均可在偏好設定內停用。 |
| ⌥⌦<br />⌥⌫ | 刪除一個字詞節點。<br />⌥⌫ 會優先刪除尚未完成的讀音/字根。 | 組字區有內容時才會生效；<br />輸入法並非處於標記模式時才會生效。 |
| ⌃⌘/ | 叫出注音反查視窗。 | 可在偏好設定內停用（預設啟用）。 |
| F1 ～ F20 | 不輸入任何內容。 | 組字區（含注拼槽）內尚有未遞交的內容時，這些按鍵會由輸入法就地攔截、並發出提示音，以免未遞交的讀音消失、也免得您誤以為該按鍵故障；未遞交內容為空時，一律放行給系統（故 F1～F20 本身的系統功能不受影響）。<br />`Fn` + 字母之類的系統熱鍵（例如表情符號選擇器 `Fn+E`）不受本行為影響。 |
...
| ⌥→<br />⌥← | 在選字窗顯示時移動組字區內的游標（以字詞節點為單位，逐節移動）。 | 橫排輸入時為 ⌥→／⌥←，縱排輸入時為 ⌥↓／⌥↑。 |
| ⇧⌥→<br />⇧⌥← | 在選字窗顯示時移動組字區內的游標（以單個幅長（讀音）為單位，逐幅移動）。 | 橫排輸入時為 ⇧⌥→／⇧⌥←，縱排輸入時為 ⇧⌥↓／⇧⌥↑；<br />如果發現移動後的游標出現了切割字元的情況的話，則會自動將游標繼續移動至當前節點的邊界。 |
