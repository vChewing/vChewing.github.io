...
為了照顧對注音符號不熟悉的使用者群體，唯音輸入法提供了拼音並擊模式。該模式可在輸入法偏好設定的「通用設定」頁籤內開啟。在拼音並擊模式下，你敲的大千注音會以漢語拼音的形式實時顯示在組字區內的組音區當中：
...
> 唯音輸入法推薦將其設定為「直接遞交小寫字母」，因為這樣可以用「Option+Shift+字母鍵」的方式直接遞交大寫字母。**然而，這要求輸入法偏好設定的「鍵盤設定」頁籤當中的「基礎鍵盤佈局」得是「Apple 標準注音/倚天注音鍵盤佈局」或者任何以「vChewing」開頭的鍵盤佈局才可以**。
